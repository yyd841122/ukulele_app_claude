...
| 范围受控 | MVP Scope 不被突破 |
| 硬门禁 100% 执行 | 所有 Agent 输出必须通过硬门禁 1-7 |
| 软建议默认要求 | 软建议 8-10 默认要求执行；简单文档修复类任务可由 Chief Architect 明确豁免 |
| 豁免必说明 | 若豁免软建议，必须在 Review Report 中说明原因 |
| 三步反思机制 | 默认要求每个 Agent 输出包含三步反思；简单文档修复类任务可豁免，但必须说明原因 |
