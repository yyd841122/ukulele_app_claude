...
> Task ID：`T049_PRODUCT_V2_PHASE1_IMPLEMENTATION_PLAN` / `T049A_FIX_IMPLEMENTATION_PLAN_REVIEW_AND_EVIDENCE`（证据补齐 + T047B 2 项 Minor 登记 TECH_DEBT）
> 上游：`docs/PRD_v2.md` v0.3（已批准）/ `docs/architecture/SDD_V2.md` v0.4（已批准）/ `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0（已批准）
> 起始 HEAD：`e1080f7`（T049）/ `0de2949`（T049A 证据补齐） | 测试基线：744（**保持 744，未重跑**） | 文档长度目标：≤ 500 行
> 最后更新：2026-06-25 | 版本：1.2
...
> 多 Agent 协作：Primary Plan Architect（主会话）/ Implementation Plan Reviewer `t049a-implementation-plan-reviewer`（subagent `ac0c16877010ab8a5`，**Approved**，**0 Findings**，**0 Blocker**）；TDD v1.0 三 Reviewer 已落盘（T048）；SDD v0.4 二元裁决闭环；T047B Flutter Data Architecture Reviewer 二元裁决闭环；不重复运行 Product Alignment Reviewer（不改变 PRD 范围）。
>
...
>
> **v1.2 修订**（T049A 证据补齐 + T047B Minor 处理）：① 头部 Reviewer 状态由 **Pending** 改为 **Approved**（Reviewer ID 落盘）；② §6 T047B F-1 / F-2 Minor **改为**登记到 `docs/dev/TECH_DEBT.md`（TD-023 / TD-024），**不再**保留"不修且不登记"的双重否定表述（消除歧义）；③ §6 显式声明 T049A 提交后 Implementation Plan 进入"Approved / 可启动任务"状态。
...
## 6. T047B 遗留 Minor 处理（T049A 决议 = 登记 TECH_DEBT，**不**顺手修）

T047B Reviewer 留下 2 项非阻断 Minor（详见 `docs/dev/AGENT_QUALITY_METRICS.md` §4.Z T047B Scorecard）。T049 v1.1 采取"**不**修 + **不**登记"双重否定表述，GPT 首席架构师复审指出该表述易被误读为"已忽略"。T049A 决议 = **登记到 `docs/dev/TECH_DEBT.md`**（TD-023 / TD-024），文档本身**不**顺手修（避免改动范围扩大）。

| ID | 内容 | 处理决议 | TECH_DEBT 登记 |
|----|------|----------|----------------|
| F-1 | `docs/dev/TASK_LEDGER.md` line 470（T047 既有条目）保留已废弃 `else if` 字样 | **不**在本 Implementation Plan 顺手修（属历史 ledger 措辞遗留，T047 既有）；登记到 TD-023（Low） | TD-023 |
| F-2 | `docs/ROADMAP.md` P4 Curriculum 行写"Drift schemaVersion=2 → 3"与实际 P4 升级 3→4 冲突 | **不**在本 Implementation Plan 顺手修（属 P4 / 不阻塞 P1）；登记到 TD-024（Low） | TD-024 |

**登记内容必含**（每项）：① 来源任务 = T047B；② 风险等级 = Low；③ 不阻塞 P1 的原因（措辞历史遗留 / P4 阶段冲突未影响 P1 schemaVersion 保持 2）；④ 后续建议处理时机（P1 关闭门后任意 P2+ 任务顺手修，或留 ROADMAP cleanup 独立任务）；⑤ 当前 Implementation Plan 如何避免误读（§6 显式记录 + TECH_DEBT 留痕 + Reviewer 已 Approved 0 Findings）。

**铁律**：T049 Implementation Plan 引用 SDD / TDD 时，**不**复制 `else if` 措辞（统一为"累进式 `if (from < N)` 链"）；引用 ROADMAP schemaVersion 时，统一用 SDD v0.4 编号（P2 = 2→3 / P4 = 3→4）。任何后续 Implementation Task 启动前必须读 T047B Scorecard + 本 Implementation Plan §6 + TD-023 / TD-024。

**T049A 提交后状态**：T047B 2 项 Minor **已闭环**（登记 TECH_DEBT + Implementation Plan 显式记录 + Reviewer Approved 0 Findings）；T049 Implementation Plan 进入"**Approved / 可启动任务**"状态。
...
| 1.2 | 2026-06-25 | T049A 证据补齐：① 头部 Reviewer 由 Pending 改为 Approved（subagent `ac0c16877010ab8a5`，0 Findings，0 Blocker）；② §6 T047B F-1 / F-2 Minor 改为登记 TECH_DEBT（TD-023 / TD-024），消除"不修且不登记"歧义；③ Implementation Plan 状态进入 "Approved / 可启动任务" |
