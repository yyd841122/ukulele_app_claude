...
### 4.28 T040_ALIGN_RELEASE_VERIFIER_WITH_V110 Scorecard（v1.1.0 T022 Release 验证器升级 + 多 Agent 协作）

| 字段 | 值 |
| --- | --- |
| Task ID | `T040_ALIGN_RELEASE_VERIFIER_WITH_V110` |
| 起始 Commit | `66508ff3b33dd3f9b8384ffa2c9a3cae5414e908`（master） |
| HEAD（最终） | `71f4f287c6a25c40d3ef52cbe1dfe23e0905d2a2`（T040 commit 1 = `test: align release verifier with v1.1.0`）+ 收口 commit 2 = `docs: record T040 verifier collaboration` |
| 任务定位 | 升级 T022 Release 验证器以正确验证 v1.1.0 真实音频版本：**不**改生产代码 / 依赖 / Manifest / Gradle / schema / 版本号 / 签名配置 |
| Exact Test Count | **720**（基线 711 + T040 新增 9 verifier 单元测试） |
| Final Approval | **APPROVED**（Primary 落地 + 1 独立 readonly Reviewer Approved + 0 Blocker） |
| Pre-commit / Commit / Push | No (启动前) / Yes (2 commit = T040 commit 1 已落地 + 收口 commit 2 落地) / No (无 push) |
| Amend / Rebase / Reset | No / No / No |
| `dart format lib` 写盘动作 | Yes (1 次，写盘产生 K 表 / `_resolveOnPath` / `_emit(...)` 三处空白整理；dart format 预期产物，**不**新增代码改动) |
| Scope Clean | Yes（**仅**修改 2 文件 + 3 doc 文件：`tool/verify_release_artifacts.dart` + `test/tool/verify_release_artifacts_test.dart` + `docs/dev/TASK_LEDGER.md` T040 节段 + `docs/dev/AGENT_QUALITY_METRICS.md` §4.28 T040 Scorecard（本段）+ `docs/dev/TECH_DEBT.md` TD-022 闭环说明段；**不**修改既有 4.1 ~ 4.27 任何 Scorecard + 既有 TD-001 ~ TD-021 任何条目 + 既有 T006-T039 任何台账条目） |
...
| **多 Agent 协作细节** | **Primary Agent**（T040 实施者）：完成 5 步反思闭环（初步实现 → 自我找茬 ≥3 项 → 终极交付）+ 落地 `tool/verify_release_artifacts.dart` 升级（删硬编码 `1.0.0/2` + 加 `_ExpectedVersion` + `parseExpectedVersion()` + `_loadExpectedVersion()` + `_requiredPermissions` + `_assertRequiredPermissions()` + header 注释升级）+ 新增 `test/tool/verify_release_artifacts_test.dart` 9 项 `parseExpectedVersion` 单元测试 + 运行定向测试 / `flutter analyze` / `flutter test` 720 / `dart run tool/verify_release_artifacts.dart` 全部通过 + commit `71f4f28` 落地。**Reviewer Agent**（1 个独立 readonly `general-purpose` Agent，agentId `a94f59ff0ddd72332`，12 次工具调用 / 38.5s / 28,254 tokens）—— 逐字执行 7 项核对（① `git diff tool/verify_release_artifacts.dart` 确认 4 项预期改动 + 无 production 改动；② `git status` 确认无超出 2 文件的修改；③ 读测试文件确认 9 项覆盖 v1.1.0+3 基线 + quoted 变体 + leading whitespace + missing/malformed + v1.0.0+2 不等 v1.1.0+3；④ 读 `pubspec.yaml` 确认 `version: 1.1.0+3`；⑤ 读 `AndroidManifest.xml` 确认 `RECORD_AUDIO` 声明 + `INTERNET` 缺席；⑥ spot-check `_forbiddenPermissions = [INTERNET]` / `_requiredPermissions = [RECORD_AUDIO]` / `_loadExpectedVersion` 实际读 `pubspec.yaml`；⑦ CRLF 提示**不**是 Blocker）→ **Approved**（无 Blocker；唯一非阻塞观察 = dart format 空白整理 318 行变更对 60 行实质改动，**不**是 Blocker，仅提示未来 T040+ 任务注意 diff 体量）。**0 Blocker** —— Reviewer **未**发现任何 Blocker。**可复用经验**：低风险工具任务（验证器脚本 + 单元测试）使用 **Primary + 1 个独立 Reviewer** 即可；**不**需要扩展到 Primary + 2+ Reviewer 或引入交叉多 reviewer；本任务验证了"less is more"协作模型对 T022-style 工具升级的适配性。**安全边界**：① **未**读取 `key.properties` / keystore / 密码 / alias / 敏感路径（验证器脚本本身**不**读取这些，与 T022 既有契约一致）；② **未**修改签名配置 / Gradle / Manifest；③ **未**修改生产代码 / 测试代码（除本任务唯一新增的 `test/tool/verify_release_artifacts_test.dart`）；④ **未** push / **未** tag / **未** amend / rebase / reset --hard；⑤ **未**声称 Release APK 已上架 / iOS 已验收 / 全 ROM 兼容 |
| T040 关键确认 | ① 精确测试数 = **720** ✓；② 验证器对 v1.1.0 APK / AAB 输出 `VERIFY_OK` ✓；③ `RECORD_AUDIO` 现为必需 + 已声明 ✓；④ `INTERNET` 仍禁止 + 缺席 ✓；⑤ `pubspec.yaml` 1.1.0+3 与 aapt 解析的 `versionName=1.1.0` / `versionCode=3` 一致 ✓；⑥ APK/AAB 签名 / Debug vs Release 证书差异 保留 ✓；⑦ **不** 修改生产代码 / 依赖 / Manifest / Gradle / schema / 版本号 / 签名配置 / `key.properties` / `.gitignore` / 构建产物 / 既有 T006-T039 任何台账条目 |
...
#### 4.33 T043 精简架构复审 Scorecard（3 个独立只读 Reviewer · 0 Blocker · 0 代码修改）

| 项目 | 内容 |
| --- | --- |
| Task ID | `T043_STRUM_PATTERN_ASSETS_AND_WIDGET` 精简架构复审 |
| 复审触发 | T043 commit `105aad9` 落地后用户本轮追加一次精简架构复审；限定 3 个独立只读 Reviewer（UI/Accessibility + Music Domain + Architecture），**禁止**启动 T044 / 禁止重构 / 禁止 amend |
| Primary Agent | `03-mobile-ui-engineer`（T043 实施者）+ 复审阶段 `00-chief-architect` 汇总 |
| Review Agent 1（UI/Accessibility） | `general-purpose` subagent，agentId `ab09acaf817be8764` / 独立只读 / **Approved with non-blocker observations** |
| Review Agent 1 关键 finding | ① Approved — Semantics label `'4/4 拍，每拍下扫一次；前两拍弹 C 和弦，后两拍弹 Am 和弦'` 准确，`ExcludeSemantics` on inner `4/4` Text 正确防重读；② Approved — Material 3 默认 light `onSurface` / `onSurfaceVariant` / `primary` 对比度充足；③ Non-blocker — per-cell Text 在 `Semantics(container: true)` 内无 `MergeSemantics` / `ExcludeSemantics` 包装 → 读屏可能轻微冗余；④ Non-blocker — widget `_kChordSequence` / `_kBeatsPerMeasure` 硬编码 vs `kBuiltInLessons[0].strumPattern` 未来可能 drift；⑤ Approved — `FittedBox(scaleDown)` + 窄屏 320x800 / 280x600 测试覆盖 |
| Review Agent 2（Music Domain） | `general-purpose` subagent，agentId `a36ca96df85c8b740` / 独立只读 / **Approved with non-blocker observations** |
| Review Agent 2 关键 finding | ① Approved — 4/4 / 4 拍 / 下扫方向 / `Am` 序列 / 60+80 BPM 步骤 / `linkedTaskIds=['day4_chord_switch']` 全部与 `docs/learning/lesson_c_am_down_4x4.md` v0.2 + `T041 §3.1 / §4.1` 对齐；② Approved — `②Am按` 修正 + "中指始终在位" 禁词扫描通过；③ Non-blocker — 与 UI/Accessibility 同源：widget 硬编码 vs 常量 drift（`strum_pattern_diagram.dart:68-76` 4 个 `_k*` 常量 + `_kSemanticsLabel`） |
| Review Agent 3（Architecture） | `general-purpose` subagent，agentId `ae8969bfdec372b3c` / 独立只读 / **Approved with non-blocker observations**（**0 Blocker**） |
| Review Agent 3 关键 finding Q1 | Non-blocker — `practice_plan_constants.dart:1-26` 明示"types in domain, data in constants"先例；T043 把 4 个 lesson types 内联到 `core/constants/lesson_constants.dart` 违反该先例。Recommendation：未来提取至 `lib/features/lesson_c_am_down_4x4/domain/`（**不在本任务执行**） |
| Review Agent 3 关键 finding Q2 | Approved — `T041 §4.1` line 83 显式 pin `core/constants/lesson_constants.dart` 槽位 → `practice_plan_constants` 先例胜出；`kBuiltInLessons` 数据文件位置合规 |
| Review Agent 3 关键 finding Q3 | Non-blocker — `features/lesson_c_am_down_4x4/` 命名匹配 lesson id 而非稳定 feature 名（contrast `chord_library/`）。Recommendation：lesson 2 上线前重命名为 `features/lessons/` 或 `features/beginner_lessons/` —— 1 处 test import 改动、widget 零改动 |
| Review Agent 3 关键 finding Q4 | Non-blocker — 缺 `domain/` slot；widget 显式解耦（line 40 仅 import `flutter/material.dart`，**未** import `lesson_constants.dart`）—— types 迁移不需要改 widget。**churn 数：types 留在原位 = 0；types 迁移 = 5**（仅 5 个 constants test import 需更新） |
| 架构结论 | T043 范围内 **0 Blocker**。`T041 §4.1` line 83 已显式 pin `core/constants/lesson_constants.dart` 数据文件位置；types 内联到 `core/constants/` 违反 `practice_plan_constants` 既有先例但**不**违反 T041 设计文档。3 个非阻塞观察（Q1 / Q3 / 跨 Reviewer widget vs constants drift）**全部是 T044 / lesson-2 时机问题** |
| 修改决策 | **NO COMMIT**。所有 3 个 Reviewer 均 Approved；T041 §4.1 显式 pin 已遵守；既有测试 732 / 0 回归；本任务不制造代码改动 |
| High Risk Areas 复审触达 | ① 领域类型位置违反既有分层先例（`practice_plan_constants.dart:1-26`）→ T044 跟进；② feature 目录命名匹配 lesson id 而非稳定 feature 名 → lesson-2 跟进；③ widget 硬编码 vs constants drift（UI + Music 双 Reviewer 共识）→ T044 应接 `kBuiltInLessons[0].strumPattern`；④ 窄屏安全（320x800 / 280x600）→ 既有 2 项测试覆盖 + `FittedBox(scaleDown)` 兜底；⑤ Semantics 完整性 → `container: true` 一句话 + `ExcludeSemantics` 防重读；⑥ 音乐事实（4/4 / 4 拍 / 下扫 / `[C,C,Am,Am]` / 切换点 / BPM 60+80）→ Music Domain Reviewer 全维度通过 |
...
| Tests Passed | 732（**未**运行 `flutter test` —— 本轮无代码改动；T043 commit `105aad9` 落地时已验证 732 tests passed，本轮复用该数字） |
| Scope Clean | Yes（**仅**修改 2 doc：`docs/dev/TASK_LEDGER.md` 追加 T043 复审节段 + `docs/dev/AGENT_QUALITY_METRICS.md` 追加本 §4.33 Scorecard；**未**修改既有 §4.1-§4.32 任何 Scorecard + 既有 T006-T042 任何台账条目 + 既有 ROADMAP / PRD / DATA_MODEL_DRAFT / T041 / T042 任何 doc / `pubspec.yaml` / `pubspec.lock` / `key.properties` / `.gitignore` / 构建产物 / 任何 Dart / 测试 / Android 配置 / Drift schema / Release 产物） |
| Command discipline violation | **No**（全程命令均为单条只读 / 单条写命令：`git status --short` / `git rev-parse --short HEAD` / `git diff --check` / `git diff --stat` / `git ls-files` / `git log -1 --oneline` / `git tag --list` / `Read` / `Write` / `Edit`；无管道、无重定向、无 `&&`、无分号、无复合命令） |
| Sensitive Files Checked | `git ls-files android/key.properties` / `*.jks` / `*.keystore` / `build/app/outputs/**` 四项均空（沿用 T043 既有验证模板） |
| Build Artifacts Tracked | N/A（**未**构建 APK / AAB；**未**生成 SVG 文件——本项目无 `flutter_svg` 依赖） |
| Final Approval | **APPROVED**（3 个 Reviewer 全部 Approved with non-blocker observations；0 Blocker；0 代码修改） |
| Collaboration Value | **Medium**（3 个独立只读 Reviewer 在 T043 commit `105aad9` 落地后立即验证，0 Blocker 反向证明 T043 的 4 项非阻塞观察（types 应迁 `domain/` / feature 目录命名 / widget 硬编码 vs 常量 drift / Semantics 子节点冗余）**全部是 T044 / lesson-2 时机问题**，**不**在本任务制造代码改动；协作模型"3 个独立只读 Reviewer + 0 Blocker → 0 代码修改"对"已完成 commit 的精简复审"任务完全适配，避免 T043 主任务范围被人为扩大） |
| 可复用经验 | ① **T043 完成后立即做精简架构复审**，可在 T044 启动前把"领域类型位置 / 目录命名 / widget vs 常量 drift"3 个观察挂上 T044 backlog，**不**打断 T043 commit；② 3 个独立只读 Reviewer 角色分工清晰（UI/Accessibility 检查 widget + Semantics + 颜色对比度 / Music Domain 检查事实正确性 + 禁词扫描 / Architecture 检查项目既有先例对齐），与 T042 v0.2 "事实 / 表达分离"模式一致；③ Architecture Reviewer 区分 **Blocker / Non-blocker** 是本任务关键 —— 0 Blocker 反向证明"4 个非阻塞观察**全部**是 T044 / lesson-2 时机问题"，避免在 T043 范围内制造代码改动；④ **T041 §4.1** 设计文档"显式 pin `core/constants/` 槽位"是 Reviewer **不能**改动的硬约束 —— Reviewer 可建议"types 应迁 `domain/`"，但**不**能直接破坏既有 pin；⑤ widget 与 constants 解耦（widget **不** import constants）使得未来 types / 数据迁移不需要改 widget —— 这是 T043 的隐性架构胜利，应在 T044 lesson page wiring 时继续保持 |
| Notes | 详见 `docs/dev/TASK_LEDGER.md` T043 精简架构复审节段；3 个 Reviewer 完整报告已通过 task-notification 事件汇总到本 Scorecard；**未**修改生产代码 / 测试代码 / 依赖 / Android 配置 / Drift schema / `pubspec.yaml` / `key.properties` / `.gitignore` / `practice_plan_constants.dart` Day 1-7 / `MetronomeController` / `MetronomeSetting` / `RecordingPracticeController` / `PracticeRecordRepository` / 任何既有 controller / repository / UI 页面 / `tool/verify_release_artifacts.dart` / `agents/*.md` / `MULTI_AGENT_WORKFLOW.md` / Release 产物 / 构建产物；**未** push / **未**创建 Tag / **未** amend / rebase / reset --hard |

#### 4.34 T044 Scorecard（T044_LESSON_PAGE_AND_NAVIGATION · 初学者第一节课页面与导航闭环）

| 项目 | 内容 |
| --- | --- |
| Task ID | `T044_LESSON_PAGE_AND_NAVIGATION` |
| 任务目标 | 基于 T041 / T042 / T043 已落地设计 + 内容 + 静态资源，交付 LessonPage + `/lessons/:lessonId` 路由 + 教学入口 + widget 强制数据驱动重构；**不**修改 Day 1-7 / `MetronomeController` / `MetronomeSetting` / `RecordingPracticeController` / `PracticeRecordRepository` / Drift schema / 依赖 / Manifest / 版本号 / 完成状态 / 数据库持久化 |
| 前置状态 | HEAD = `aefd642`（T043 架构复审 commit `docs: record T043 architecture review`）；`git status --short` 空；`git ls-files android/key.properties` / `*.jks` / `*.keystore` / `build/app/outputs/**` 四项均空；测试基线 732 已稳定 |
...
| Primary 交付 | ① 新建 4 个文件（`lesson_controller.dart` / `lesson_page.dart` / `lesson_intro_card.dart` / `lesson_page_test.dart`）；② 修改 3 个文件（`router.dart` 加 `/lessons` 顶级路由 + 子路由 `:lessonId` + 父级 `redirect: (_) => '/'` 兜底 / `chord_detail_page.dart` 仅 `chord.id == 'c'` 分支嵌入 `LessonIntroCard` / `strum_pattern_diagram.dart` 数据驱动重构）；③ 修改 2 个测试（`strum_pattern_diagram_test.dart` 构造更新 + `chord_detail_page_test.dart` `setSurfaceSize(800, 1600)` + 滚动 + 入口断言）；④ 2 处 doc 收口（TASK_LEDGER + §4.34 Scorecard） |
| T043 架构复审 Q1 widget drift 修复 | T043 widget `_kChordSequence` / `_kBeatsPerMeasure` / `_kTimeSignature` / `_kSemanticsLabel` 硬编码 vs `kBuiltInLessons[0].strumPattern` 可能 drift → T044 改为 `required StrumPattern strumPattern` 参数 + 4 个字段全部从 pattern 派生；现有 7 项 widget test 通过 `kBuiltInLessons.first.strumPattern` 传递 → 零硬编码副本。Q2（types 迁 `domain/`）/ Q3（目录命名）按 T043 Reviewer 共识**不**在 T044 范围 |
| Primary 自检 7 项风险全部缓解 | ① widget 强制数据驱动（Q1 fix）；② `LessonIntroCard` 仅 `/chords/c` 显示（T041 §7 R-01 缓解）；③ 空 / 未知 id 双分支走 `_LessonNotFound`，不抛 `StateError`；④ 节拍器按钮文案"调整到 N BPM 后开始"明示用户手动调 BPM，**不**改 domain；⑤ 整页 `ListView` + widget 内部 `FittedBox(scaleDown)` 防溢出；⑥ 标题 `Semantics(header: true)` + step 序号 `CircleAvatar` / `Step N` 文本 `ExcludeSemantics` 防读屏连读；⑦ `_LessonStepTile` 用 `Wrap` 替代 `Row` 让按钮可折行，`_LessonChordRow` 容纳 C / Am 引用图 |
| Tests Passed | **738**（基线 732 + T044 新增 6 项 lesson_page_test；既有测试未减 0 回归） |
| Scope Clean | Yes（仅 4 个新文件 + 3 处修改 + 2 处 doc 收口；**未**修改既有 §4.1-§4.33 任何 Scorecard + 既有 T006-T043 任何台账条目 + 既有 ROADMAP / PRD / DATA_MODEL_DRAFT / T041 / T042 / T043 任何 doc / `pubspec.yaml` / `pubspec.lock` / `key.properties` / `.gitignore` / 构建产物 / 任何现有 controller / repository / Drift schema / Android 配置 / `MetronomeSetting`） |
| Command discipline violation | No（全程命令均为单条只读 / 单条写命令：`git status --short` / `git diff --check` / `git ls-files` / `dart format` / `flutter analyze` / `flutter test` / `Read` / `Write` / `Edit`；无管道、无重定向、无 `&&`、无分号、无复合命令） |
| Sensitive Files Checked | `git ls-files android/key.properties` / `*.jks` / `*.keystore` / `build/app/outputs/**` 四项均空（沿用 T043 既有验证模板） |
| Build Artifacts Tracked | N/A（**未**构建 APK / AAB；**未**生成 SVG 文件——本项目无 `flutter_svg` 依赖） |
| Manifest / Deps / Schema / Version unchanged | ✓ `pubspec.yaml` version 仍为 `1.1.0+3`；✓ `pubspec.lock` 未改；✓ `lib/data/database/app_database.g.dart` 未改（schemaVersion = 1）；✓ 三处 `AndroidManifest.xml` 未改；✓ 无新依赖 / 无新权限 / 无新字段 |
| Final Approval | **APPROVED**（Primary 7 项自检风险全部缓解；T043 架构复审 Q1 widget drift 已修复；0 回归 738 tests passed；2 个独立 Reviewer Approved / 0 Blocker；`MetronomeController` / `MetronomeSetting` / `RecordingPracticeController` / `PracticeRecordRepository` / Drift schema 零修改；6 项 Non-blocker 观察均为 nice-to-have，挂 lesson-2 polish backlog） |
| Review Agent 1（Flutter Architecture/UI） | `general-purpose` subagent，agentId `abe2870881dd1f4b7` / 独立只读 / **Approved**（0 Blocker / 3 Non-blocker） |
| Review Agent 1 关键 finding | ① Non-blocker — `lesson_page.dart:198` `_LessonChordRow` 硬编码 `['c', 'am']`，可接受（仅 1 个已发布课程；未来 lesson-2 切片时可改为 `for (chordId in lesson.strumPattern.chordSequencePerBeat.distinct)` 自动跟随）；② Non-blocker — `strum_pattern_diagram.dart:141` `*` chord-switch marker 用 `chordSequence.length >= 4` 而非 `chordSequence[0] != chordSequence[2]`，minor；③ Approved — +1075 line budget 合理，无 UI 重画 `ChordDiagram`（复用既有 widget），painter 仅 1 个 `CustomPaint`，page 主体为 layout glue，**无** scope creep |
| Review Agent 2（Beginner UX） | `general-purpose` subagent，agentId `afcdff416a6adda90` / 独立只读 / **Approved**（0 Blocker / 3 Non-blocker） |
| Review Agent 2 关键 finding | ① Non-blocker — `lesson_intro_card.dart:57` "本节可选" 仅出现在 `Semantics(container: true, label)` 中；肉眼用户看到 `primaryContainer` filled card 无 visible "可选" badge，sighted / screen-reader 体验**不**对等；② Non-blocker — `lesson_page.dart:279-284` "Step N" header 包 `ExcludeSemantics` 后，screen reader 失去 "instruction belongs to step 3" 父子关系（建议 per-tile `Semantics(header: true)` 或在 instruction 前补 step 号）；③ Non-blocker — `lesson_page.dart:299-307` 跳转按钮文案"调整到 60 BPM 后开始" / "录音复盘"**未**明示"完成后按返回回到本课"，system back button 可用但 polish 缺失 |
| Reviewer 决策 | **6 项 Non-blocker 全部挂 lesson-2 polish backlog**，本任务**不**制造代码改动；理由与 T043 复审（§4.33）一致：0 Blocker → 0 代码修改；T044 范围内 widget 数据驱动重构 + 路由 + 入口 + 6 项 test 已闭环，polish 项不属本次目标 |
| Collaboration Value | **Medium**（Primary 自检 7 项 + T043 Q1 一次性修复 + 2 个独立 Reviewer Approved / 0 Blocker / 6 项 nice-to-have；协作模型"Primary 自检 + 2 个独立 Reviewer 验证"对"widget 数据驱动重构 + 路由 + 页面 + 入口"型任务适配；6 项 polish 观察明确归属 lesson-2 而非 T044 / T045，避免范围扩大） |
| 可复用经验 | ① T043 架构复审挂的 Non-blocker 观察应在紧邻的下一个任务（T044）就消化，**不**要拖到 lesson-2 时机（widget 数据驱动 + 测试构造同步更新 = 0 风险）；② go_router 顶级路由**必须**有 `builder` / `pageBuilder` / `redirect` 三选一，否则集成测试（全栈 `UkuleleApp` mount）会全挂——T044 集成测试一次性发现 6 个失败后用 `redirect: (_) => '/'` 兜底修复；③ 既有 widget test 的"默认 800x600 视口"假设遇到新加内容（如 `LessonIntroCard`）时**必须**升级为 `setSurfaceSize` + `scrollUntilVisible`，否则 description 被挤出视口导致 false-negative；④ T041 §7 R-01 "多入口困惑" 在 widget 层 hardcode 入口白名单（`chord.id == 'c'`）比依赖配置文件更稳；⑤ 6 项 Non-blocker polish（visible "可选" badge / per-tile Semantics header / "完成后返回继续" hint）一次性记入 ledger，**不**在 T044 制造代码改动，**不**在 T045 误扩大范围——lesson-2 polish backlog 接管；⑥ navigation matrix 5 项（`/lessons/:id` push / `/metronome` push / `/recording` push / `/` go-back / 返回 nav AppBar auto）全部由 widget 静态可见，**不**需要 controller / repository / query 参数介入，符合 T044 "最小必要修改" 原则 |

#### 4.35 T045A Scorecard（T045A_FIX_LESSON_CHILD_ROUTE_REDIRECT · 路由父 redirect 不拦截子路由 FIX）

| 项目 | 内容 |
| --- | --- |
| Task ID | `T045A_FIX_LESSON_CHILD_ROUTE_REDIRECT` |
| 任务目标 | 修复 `lib/app/router.dart` `/lessons` 父级 `redirect: (_, __) => '/'` 在 go_router 17.x 下同时拦截子路由 `/lessons/:lessonId` 的 production bug；新增用真实 `appRouter` 驱动的端到端回归测试。**不**修改其它路由 / **不**调整课程功能 / **不**修改 T044 已交付的 widget / controller / 入口卡片 |
| 前置状态 | HEAD = `6b57669d8a2f96afb60c0ce28e2ffd754288b112`（T044 复审 commit）；`git status --short` 仅 2 个 T045 新文件（`docs/qa/T045_LESSON_ANDROID_ACCEPTANCE.md` + `test/features/lesson_c_am_down_4x4/presentation/lesson_navigation_isolation_test.dart`）；测试基线 740 |
| Bug 根因 | go_router 17.x 在解析子路由 `/lessons/:lessonId` 时**仍**调用父级 `/lessons` 的 `redirect`（`_processRouteLevelRedirects` 通过 `matchList.visitRouteMatches` 遍历整棵 match 树）；父级无条件返回 `'/'` → `context.push('/lessons/c_am_down_4x4')` 被静默重写为 `'/'` → `LessonPage` 永远不挂载 → 用户被推回 HomePage。T044 在 router 注释中错误假设父 redirect 仅在直接访问 `/lessons` 时生效 |
| 修复方案 | 父级 redirect 改为按 `state.uri.path` 区分：`'/lessons'` 自身 → 返回 `'/'`；其它（含子路由 `/lessons/<id>`）→ 返回 `null` 让子路由 builder 正常挂载。未知 lessonId 由 `LessonPage._LessonNotFound` 内部渲染（保留"受控错误页"语义） |
| Primary 交付 | ① 修改 1 个文件：`lib/app/router.dart`（`/lessons` 父 redirect 改为按 `state.uri.path` 区分，+30 / -11 行）；② 新增 1 个文件：`test/integration/lesson_route_e2e_test.dart`（4 项 e2e：直接 push 有效 id / 直接 push 未知 id / 直接访问父级重定向 / C 和弦页点击"开始课程"）；③ 既有测试 doc 增强：`test/features/lesson_c_am_down_4x4/presentation/lesson_navigation_isolation_test.dart` 顶部 doc 增加"不经过生产 appRouter"声明；④ 2 处 doc 收口：`docs/qa/T045_LESSON_ANDROID_ACCEPTANCE.md` 追加 T045A 节段 + `docs/dev/TASK_LEDGER.md` 追加 T045A 节段 |
| Primary 自检 4 项风险全部缓解 | ① **Redirect 域判定**：用 `state.uri.path` 而非 `matchedLocation`（实测 `matchedLocation` 在父级 redirect 中对子路由访问也返回 `/lessons`，无法区分）；② **e2e 数据库耦合**：避免在测试 mount `UkuleleApp`（`HomePage` 通过 `todayPracticeControllerProvider` 读 Drift DB → `pumpAndSettle` 死循环），改为 `MaterialApp.router(routerConfig: appRouter)` 直接驱动并选择 `initialLocation` 跳过 HomePage；③ **`appRouter` 全局单例状态泄漏**：每个测试 `setUp` 先 `appRouter.go('/')` 重置；④ **`pumpAndSettle` 在 push 后挂起**：实测 go_router 17.x `appRouter.routerDelegate.currentConfiguration.uri.path` 在 push 后异步更新（widget tree 同步更新），tap 场景断言 widget 挂载即可，URL 字符串由"直接 push"测试 pin |
| Tests Passed | **744**（基线 740 + T045A 新增 4 项 e2e；既有 740 全部保留 0 回归；`lesson_navigation_isolation_test.dart` 2 / 2 仍过） |
| Scope Clean | Yes（仅 1 处生产代码修改 + 2 个新文件 + 1 个既有测试 doc 增强 + 2 处 doc 收口；**未**修改其它路由 / 课程功能 / 既有 740 项测试 / Manifest / Gradle / Drift schema / 依赖 / 版本号 / 既有 doc 主体 / Release 产物 / `agents/*.md` / Day 1-7 任何任务 / 完成状态 / 数据库持久化 / 节拍器 / 录音 controller / `MetronomeSetting`） |
| Command discipline violation | No（全程命令均为单条只读 / 单条写命令：`git status --short` / `git diff --check` / `flutter analyze` / `flutter test` / `Read` / `Write` / `Edit`；无管道、无重定向、无 `&&`、无分号、无复合命令） |
| Sensitive Files Checked | `git ls-files android/key.properties` / `*.jks` / `*.keystore` / `build/app/outputs/**` 四项均空（沿用既有验证模板） |
| Build Artifacts Tracked | N/A（**未**构建 APK / AAB；修复提交后由用户在真机执行 `adb install -r` 覆盖安装） |
| Manifest / Deps / Schema / Version unchanged | ✓ `pubspec.yaml` 未改（version 仍为 `1.1.0+3`）；✓ `pubspec.lock` 未改；✓ `lib/data/database/app_database.g.dart` 未改（schemaVersion = 1）；✓ 三处 `AndroidManifest.xml` 未改；✓ 无新依赖 / 无新权限 / 无新字段 |
| Final Approval | **PASS（真机 2026-06-24）**（自动化 744 / 744 + analyze 0 issue + 用户在原设备 `adb install -r` 覆盖安装后点击"开始课程"确认课程页真实显示 + 返回箭头 / 节拍器入口 / 录音入口 全部按预期行为，无自动启动或状态污染；数据保留项保持 **N/A** 不改写） |
| Review Agent 1（Flutter Router） | `general-purpose` subagent / 独立只读 / Blocker 修复后复审（见 Reviewer 协作节） |
| Reviewer 协作 | T045A 范围内由 1 个独立 Flutter Router Reviewer（只读）复审 4 项：(a) redirect 域判定（`uri.path` vs `matchedLocation` / `fullPath`）是否符合 go_router 17.x 语义；(b) 4 项 e2e 是否覆盖 task brief 全部 4 项场景（直接 push 有效 id / 直接访问父级 / 未知 lessonId / C 和弦点击入口）；(c) 是否误改了其它路由或课程功能；(d) 数据保留口径是否如实记录（N/A，不得声称 PASS） |
...
### 4.Z T047B Scorecard（Product V2 Drift 跨版本累进式迁移设计勘误）

| 字段 | 值 |
| --- | --- |
| Task ID | `T047B_FIX_MULTI_VERSION_DRIFT_MIGRATION_DESIGN` |
| Primary Agent | 主会话（Chief Architect） |
| Review Agents | Flutter Data Architecture Reviewer `t047b-flutter-data-reviewer`（r1 **Blocker** → final **Approved**；**仅 1 名**真实独立只读 Reviewer，对应任务要求"只配置 1 名真实、独立、只读的 Flutter/Data Architecture Reviewer"；Audio / Product Reviewer 本次不重复运行——本次仅改 Drift schemaVersion 策略边界，不改音频时间基准 / 不改 PRD 范围） |
| High Risk Areas | `onUpgrade` 跨多版本直接升级路径完整性 / 累进式 `if (from < N)` 链 vs `else if` 精确分支对语义边界 / `from=1, to=3` 跨版本合法升级路径 / 未来 P4 v3→v4 同样规则可扩展性 / T048 TDD 测试矩阵覆盖度（v1 升级 v3 / 重复 open 幂等 / 6 列一致性 / 迁移失败回滚 / 真实 fixture 来源） / `StateError` 兜底与合法 `from/to` 边界 |
| Blockers Found (r1) | 1：① F-1 Blocker = v0.3 §3.8(b) `onUpgrade` 用 `if / else if` 精确分支对（`if (from == 1 && to == 2) return;` + `else if (from == 2 && to == 3) { createTable(scores); }`）**只能处理相邻版本**；v1.1.0 既有 schemaVersion=1 / 2 用户覆盖安装 P2 新二进制时 `from=1, to=3` 既不命中 1→2 也不命中 2→3，落入 `StateError` 兜底，合法升级路径崩溃；T048 TDD 4 项硬门缺"v1 fixture 升级 v3"用例，无法在测试中捕获此漏迁移；未来 P4 v3→v4 同样落入 `else if` 链盲区（`from=2, to=4` 漏 3→4 迁移） |
| Blockers Valid | 1 / 1（**v0.3 文档明确要求 `if / else if` 链 + `throw StateError` 兜底**，与"覆盖 `from=1, to=3` 跨版本直接升级"目标直接矛盾，**v0.4 必须修正**） |
| Blockers Found (final) | 0 |
| Findings Found (r1) | 5：F-1 Blocker + F-2 to F-5 Minor：F-2 = §3.8 T048 TDD 硬门缺"v1 fixture 升级 v3"路径，仅覆盖新安装 + v2→v3 + 重复 open + 迁移失败回滚 4 项；F-3 = §3.8 TDD 硬门未要求验证 `scores` 表 6 列 / 约束 / 索引与 drift_dev `schema_dump.dart` 三方一致性（`sqlite_master` + `pragma table_info` + generated schema）；F-4 = §3.8 TDD 硬门未明确"fixture 必须来自对应历史 schema 真实 SQLite 文件或可信历史数据库快照"，可能被未来 Agent 用"空数据库 + `PRAGMA user_version = 1`"伪造 v1 既有用户；F-5 = §5 Drift 升级铁律 footer 写"P4 再统一迁移到 schemaVersion=3"与 §3.8 / §3.9 编号冲突（P2 已先占 v3，P4 必须 v4） |
| Findings Valid | 5 / 5（全部采纳；v0.4 一次性合并修订） |
| Fix Commits Required | 0（v0.4 一次性合并修订；Reviewer 在文档内闭环） |
| Tests Passed | 744（基线保持；新增 / 更新 / 删除测试 0 / 0 / 0；不写实现代码） |
| Scope Clean | Yes（仅修改 `docs/architecture/SDD_V2.md` v0.3 → v0.4 + `docs/dev/TASK_LEDGER.md` 追加 T047B + `docs/dev/AGENT_QUALITY_METRICS.md` 追加 §4.Z T047B Scorecard + `docs/ROADMAP.md` 修订 §"T047 后 Product V2 路线" + §3.8 schemaVersion 策略表 v0.4 勘误行） |
| Command discipline violation | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| Final Approval | 待 GPT 复审 |
| Collaboration Value | **High**（T047B 二元裁决闭环机制在"单 Reviewer / 单 Blocker"场景下实证有效：① Flutter r1 → Blocker（F-1 `if / else if` 精确分支对只能处理相邻版本，跨版本合法升级会落空）→ 主会话一次性合并修正 → Flutter final **Approved**；最终 0 Blocker；Reviewer 准确识别 v0.3 `onUpgrade` `else if` 链 + `StateError` 兜底在 `from=1, to=3` 场景下的真实崩溃路径，并指明未来 P4 `v3→v4` 同样盲区 + T048 TDD 缺 v1 升级路径 + 6 列一致性 + 真实 fixture 来源 + schemaVersion 编号冲突 4 项 Minor 全部落地；T047B 4 项 F-2 / F-3 / F-4 / F-5 Minor 与 1 项 F-1 Blocker 共同构成"累进式迁移设计完整性"五维校验矩阵；Reviewer 报告完整按 `AGENT_REVIEW_TEMPLATE.md` 模板填写 Scope Reviewed / Evidence Checked / Findings / Verdict；多 Agent 二元裁决闭环机制在"单 Reviewer 真实独立只读"任务边界下产生真实价值；`less is more` 协作模型在 Drift schemaVersion 边界修正任务上再次验证"1 名真实独立只读 Flutter/Data Architecture Reviewer + Primary 主会话"足够闭环，避免不必要的 Audio / Product Reviewer 重复运行） |
| Notes | ① T047B v0.4 核心勘误落地：① §3.8(b) `onUpgrade` 从 v0.3 `if / else if` 精确分支对改为**累进式（cumulative）`if (from < N)` 链**（每个版本独立 `if (from < targetVersion)` 块，互不互斥，跨版本跳级自动按顺序执行所有未应用迁移），明确覆盖 `from=1, to=3` 跨版本直接升级路径；P2 实际启动时把 2→3 作为第二个独立 `if (from < 3)` 块追加（**不**改既有 1→2 分支、**不**替换为 `else if`），不再要求 `StateError` 兜底覆盖合法 `from/to`；② §3.8 T048 TDD 验收硬门从 v0.3 4 项扩为 v0.4 8 项：**(1)** 新安装走 `onCreate` 创建 v3 schema + **(2)** v1 fixture 升级到 v3 + **(3)** v2 fixture 升级到 v3 + **(4)** v3 数据库重复打开 + **(5)** 验证 `scores` 表 6 列 / 约束 / 索引与 drift_dev generated schema 三方一致 + **(6)** 注入迁移失败回滚 + **(7)** fixture 来自真实历史 schema SQLite 文件或可信快照（**不**得仅用 `PRAGMA user_version = N` 伪造） + **(8)** 每条升级路径均验证旧表与旧数据完整保留；③ §5 Drift 升级铁律 footer 从"P4 再统一迁移到 schemaVersion=3"修正为"schemaVersion=4"（与 §3.8 / §3.9 编号对齐；F-5 Resolved） + 显式化累进式不可用 `else if` 铁律（F-1 Resolved 关键证据）+ 未来 v_n→v_n+k 通用规则；④ §8 Drift 未来迁移边界 / §10.1 初步实现 / §10.2 #2 风险 / §10.3 终极交付全链路同步显式"累进式 + 不得用 `else if`" + 跨版本升级覆盖；⑤ §11 文档版本表追加 v0.4 条目（含日期 2026-06-25 + 修改内容 + Reviewer 裁决）；⑥ 头部 Task ID / 起始 HEAD / 状态行 / 多 Agent 协作 Reviewer 行同步更新；⑦ ROADMAP §"T047 后 Product V2 路线" 段更新为 v0.4 + 累进式 + 不再用 `else if`；ROADMAP §3.8 schemaVersion 策略表行从 v0.3 改为 v0.4 勘误；⑧ 不运行 Flutter 测试或构建；⑨ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）；⑩ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑪ 下一阶段建议 `T048_PRODUCT_V2_PHASE1_TDD` + 8 项硬门启动依据本 SDD v0.4 + T047B Scorecard |

---

### 4.AA T048 Scorecard（Product V2 Phase 1 TDD）

| 字段 | 值 |
| --- | --- |
| Task ID | `T048_PRODUCT_V2_PHASE1_TDD` |
| Primary Agent | 主会话（Chief Architect） |
| Review Agents | ① Audio Test Strategy Reviewer `t048-audio-test-strategy-reviewer`（subagent `a9f2d2dd31d8fcd6b`；r1 → final **Approved**）；② Flutter State Data Reviewer `t048-flutter-reviewer`（r1 **Blocker** → v1.0 修复 → final **Approved**）；③ Product Alignment Reviewer `t048-product-alignment-reviewer`（subagent `a7f4d64a749b97200`；r1 → final **Approved**）。**3/3 真实独立只读 Reviewer**（Audio + Flutter + Product 三维全独立审查；**不**带 Audio / Product / Flutter Reviewer 重复运行）。 |
| High Risk Areas | P1 Foundation 范围与 PRD v0.3 / SDD v0.4 对齐 / OP-1 A 方案 Spike 可执行性 + 7 项必含清单 / OP-2 调音器精度 30 段录音协议 / Lesson Session 状态机唯一真相源 + dispose 责任 / Drift schemaVersion=2 保持 + 累进式 `if (from < N)` 链 P2 触发 / 既有 T027-T038B 录音契约 100% 保留 / Day 3/5/6/7 课程冻结 / 永久 Out 边界 / INTERNET 禁令 / T047B 2 项 Minor 不顺手修 / 5 分钟相对漂移 ≤ 50 ms 初始验收目标非永久常量 / 不可验证项标注 |
| Blockers Found (r1) | 1：F-1 Blocker（Flutter Reviewer）= TDD §6.2.3 §1 §2.3 §7.2 TC-4 §9.1-T8 全部引用"T012 既有 SharedPreferences 包装层"作为 settings 持久化路径，但 (a) pubspec.yaml 无 `shared_preferences` 依赖；(b) 既有 settings 持久化是 T013.2 落地的 Drift-backed `UserSettingsRepository`（`lib/shared/repositories/user_settings_repository.dart` + `drift_user_settings_repository.dart`）；(c) T012 是"实现录音回放基础版"——**不**引入 SharedPreferences；(d) TDD §1 line 8 自称"no new top-level dependencies"——与 §6.2.3 矛盾。 |
| Blockers Valid | 1 / 1（**v0.1 文档明确要求 T012 SharedPreferences 包装层** + **与"no new deps"自相矛盾** + **未来 Implementation Plan 必被误导**；**v1.0 必须修正**） |
| Blockers Found (final) | 0 |
| Findings Found (r1 Audio) | 0（12 项必含检查全部通过；直接 Approved） |
| Findings Found (r1 Flutter) | 5：F-1 Blocker + F-2~F-4 Major + F-5~F-8 Minor（详见 v1.0 Notes） |
| Findings Found (r1 Product) | 0（12 项检查全部通过；直接 Approved） |
| Findings Valid | 8 / 8（1 Blocker + 4 Major + 3 Minor；全部采纳；v1.0 一次性合并修订） |
| Fix Commits Required | 0（v1.0 一次性合并修订；3 Reviewer 在文档内闭环） |
| Tests Passed | 744（基线保持；新增 / 更新 / 删除测试 0 / 0 / 0；不写实现代码） |
| Scope Clean | Yes（仅新建 `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0 / 13 节 + 仅修改 `docs/dev/TASK_LEDGER.md` 追加 T048 条目 + `docs/dev/AGENT_QUALITY_METRICS.md` 追加 §4.AA T048 Scorecard + `docs/ROADMAP.md` 追加 T048 行） |
| Command discipline violation | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| Final Approval | 待 GPT 复审 |
| Collaboration Value | **High**（T048 三 Reviewer 二元裁决闭环实证有效：① Audio r1 → 直接 **Approved**（12/12 必含检查全部通过，含 OP-1 A 方案保持"首选 Spike 验证候选非已证明架构" / PCM chunk vs 设备采集时间区分 / 算法未锁定 / 5 分钟漂移初始目标非永久常量 / OP-1 7 项必含 / A/C/D 判定矩阵 + E 反推荐 / OP-2 30 段录音 × 4 弦 × 3 时长 / TC-13/14/15 真机边界 / 4 类时钟区分 / OP-1 ADR 强制 / 未启动 Spike / 未引入新依赖 12 项）；② Flutter r1 → Blocker（F-1 SharedPreferences 不存在 / T013.2 既有 `UserSettingsRepository` 未被识别）+ 4 Major + 3 Minor → 主会话一次性合并修正 → Flutter final **Approved**；③ Product r1 → 直接 **Approved**（12/12 检查全部通过，含 P1 范围一致 / 4 项 §7.2 P0 补齐 / OP-1 + OP-2 spike / P1 关闭门 / Day 3/5/6/7 课程冻结 / 节奏铁律 Demo A/B/C 三组 / 28/32 对齐初始基线 / 永久 Out 不变 / INTERNET 禁令 / T047B 2 项 Minor / 不可验证项 / TDD vs PRD 冲突扫描）；最终 3/3 Approved，0 Blocker，0 Approved with Conditions；3 Reviewer 在不同维度独立识别出真实风险，1 Blocker 修复闭环 + 4 Major + 3 Minor 全部采纳，证实"3 Reviewer 真实独立"在 TDD 高风险架构任务上的真实价值；`less is more` 协作模型在 TDD 任务上验证"3 Reviewer + Primary 主会话"足够闭环，避免不必要的额外 Reviewer 重复运行） |
| Notes | ① T048 v1.0 全部 8 项 Findings 修复落地：① F-1 Blocker = §6.2.3 改用 T013.2 Drift-backed `UserSettingsRepository`（pubspec.yaml 无 `shared_preferences` 依赖，**不**引入新顶层依赖）；§1 需求追踪表 / §2.3 UI 展示模型 SettingsPage / §7.2 TC-4 / §9.1-T8 同步更新（全部去掉 SharedPreferences 引用 + 明确 P1 仅内存包装层 + Drift 列新增延后 P2 升级 v2→v3 时落地）；② F-2 Major = §6.4 明确 `MetronomeSettings` 字段 KEEP + `MetronomeState` 仅扩展 `volume` 字段（KEEP/ADJUST 边界明确）；③ F-3 Major = §2.2 MetronomeSetting 字段更正（`bpm/minBpm/maxBpm/beatsPerBar/soundEnabled` —— **不**含 `volume/isPlaying`）；`volume` / `isPlaying` 拆分到 `MetronomeState`；④ F-4 Major = §6.4 既有契约保护完整列出 T031 / T031C / T031E / T031I / T032 / T033 / T034 / T035 / T035A / T035B / T038B 全部 11 项；⑤ F-5 Minor = §3.1 TunerController 标 REFACTOR（沿用 T011 既有 `TunerState`）+ §6.4 显式说明既有 state 形状 KEEP + 新增 `TunerReading` 流（与既有 `TunerState` 并存）；⑥ F-6 Minor = §7.2 TC-3 Fake 边界标 P1 NEW（`FakeMetronomeAudioSource` 与 §6.2.2 同步新建）；⑦ F-7 Minor = §6.2.2 TickEvent 更正为 `(状态发布 + tickForTesting 测试入口)` + `BeatTick` 流是 P1 NEW T2 阶段；⑧ F-8 Minor = §3.2 dispose 措辞软化（"遵循 T038B 不调 `service.dispose` 原则"代替"100% 一致"）；② §2.2 编译期护栏（禁止引入 P2 模型 OnsetEvent / LessonSessionState / ScoreTimelineEngine / FeedbackEngine / PcmStreamAudioRecorderGateway）；③ §6.2.2 MetronomeAudioSource 与既有 MetronomeController 边界（既有 T010 不持音频句柄；新接口独立 Provider 持有；volume 字段仅作用于 MetronomeState 不改 MetronomeSettings 值类型）；④ §6.5 P2 LessonSessionController 抽象契约（仅契约不实现；P1 不得引入；唯一真相源护栏）；⑤ §9.2 节奏铁律 Demo A / B / C 三组显式分组（每组 3 任务 + 真机可见产物 + 门禁）；⑥ §10.3 T047B 2 项 Minor 显式处理（不登记新 TECH_DEBT + 跟踪位置 + 不阻塞 P1）；⑦ §11 多 Agent 协作 3/3 Approved 闭环（Audio + Flutter + Product 真实独立只读）；⑧ 不运行 Flutter 测试或构建；⑨ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）；⑩ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑪ 下一阶段**只**能是独立的 Product V2 Phase 1 Implementation Plan，由 GPT 首席架构师出具独立 Prompt 后才能启动 |

### 4.AA T049 Scorecard（Product V2 Phase 1 Implementation Plan — v1.0 / v1.1 文档本体）

| 维度 | 内容 |
|------|------|
| **Task ID** | `T049_PRODUCT_V2_PHASE1_IMPLEMENTATION_PLAN` |
| **起始 HEAD** | `e1080f7`（TDD v1.0 提交） |
| **最终 commit（v1.1）** | `0de2949 docs: plan product v2 phase 1 implementation` |
| **文档版本** | v1.0（444 行）→ v1.1（444 行；三步反思修订） |
| **Scope Reviewed** | `docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.0 / v1.1（11 节 / 444 行）；TDD v1.0 §9 候选工作包；SDD v0.4 §3 / §4 / §6 / §7 / §8；PRD v0.3 §5.2 / §6 / §7.2 / §11 |
| **Evidence Checked** | (1) §0 P1 边界铁律 7 项（不扩展 Day 3/5/6/7 / 不实现 P2 闭环 / 不把双 record 实例写成已证明 / 不锁定算法 / 不引入 INTERNET / schemaVersion=2 保持）；(2) §2 T1-T11 11 个任务每任务 12 字段齐全（Task ID / 目标 / 输入 / 可改文件 / 禁止事项 / 边界 / 测试 / 真机 / Reviewer / DoD / 失败回滚 / commit 权限）；(3) §3 依赖顺序（OP-1 ADR → T8 / OP-2 ADR → T8 / m4a 闭环 / Day 3+ 冻结）；(4) §4 Demo A/B/C 三组真机可见；(5) §5 风险分级（Low 1 / Med 2 / High 3 Reviewer）+ 二元裁决铁律；(6) §6 T047B 2 项 Minor 处理（v1.1 双重否定表述，存在歧义风险）；(7) §7 P1 关闭门 8 项验收条件 + Drift schemaVersion=2 + Manifest INTERNET 禁令；(8) v1.1 修订：T1/T2 真机实验 vs ADR 区分 / T9 P1/P2 边界 / T11 降级路径 PASS 语义 / §3.2 并行现实化 / §4 Demo C 时间窗警告 |
| **High Risk Areas** | (1) T047B 2 项 Minor "不修 + 不登记 TECH_DEBT" 双重否定易被误读为"已忽略"；(2) 头部 Reviewer 状态标 `Pending` 与最终报告 "Remaining Blockers：0" 自相矛盾；(3) TASK_LEDGER / AGENT_QUALITY_METRICS 未追加 T049 任务证据；(4) T1 / T2 任务命名易被误读为"写 ADR 文档"而非"跑真机 Spike"；(5) T9 Settings 内存包装层 vs P2 v2→v3 升级列扩展边界 |
| **Blockers Found** | 0（v1.1 文档本体在提交时**未**配置 Reviewer；T049 v1.0 提交**未**走完二元裁决闭环） |
| **Findings (GPT 首席架构师复审)** | 3 项 (1) Implementation Plan Reviewer 状态 `Pending` 与"0 Blocker"矛盾 — **要求启动真实只读 Reviewer 出二元裁决**；(2) T047B 2 项 Minor "不修且不登记"双重否定易被误读 — **要求明确登记 TECH_DEBT**；(3) TASK_LEDGER / AGENT_QUALITY_METRICS 未追加 T049 任务证据 — **要求三 doc 同步** |
| **Findings Valid** | 3 / 3（均命中 Implementation Plan 文档结构与流程缺陷；T049A 接力修正闭环） |
| **Findings Resolved** | 0 / 3（T049A `T049A_FIX_IMPLEMENTATION_PLAN_REVIEW_AND_EVIDENCE` 接力；**不**在 T049 顺手修，避免越界） |
| **Scope Clean** | Yes（仅新建 `docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.0 → v1.1 / 11 节 / 444 行；**未**修改生产代码 / 测试 / 依赖 / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）/ `lesson_constants.dart` / `practice_plan_constants.dart`（Day 3/5/6/7 课程仍冻结）/ `router.dart` / `docs/PRD_v2.md` / `docs/architecture/SDD_V2.md` v0.4 / `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0） |
| **Command discipline violation** | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| **Final Approval** | **未通过**（GPT 首席架构师复审；3 项 Findings 需在 T049A 接力修正闭环） |
| **Collaboration Value** | **Medium**（T049 文档本体质量达标 — 11 任务 12 字段齐全 / 依赖显式 / Demo A/B/C 节奏 / 风险分级 + 二元裁决铁律 / 8 项关闭条件 / 5 项 v1.1 修订真实有价值；GPT 复审发现的 3 项 Findings 属"流程闭环性"而非"内容正确性" — 文档内容符合 PRD / SDD / TDD 边界；3 项 Finding 由 T049A 接力修正，避免在 T049 越界；`less is more` 协作模型在 Implementation Plan 任务上验证"T049 文档 + T049A 证据补齐"两阶段闭环比单阶段合并更清晰） |
| **Notes** | ① T049 文档本体 v1.0 → v1.1 三步反思 5 项修订全部有价值：① T1 / T2 真机实验主体 vs ADR 收口产物区分；② T9 P1/P2 边界（内存包装层 vs P2 v2→v3 升级列扩展）；③ T11 降级路径 PASS 语义（±20 cents 实验性 / 静态表 / OP-1 C/D 都属 PASS 但 PHASE1_GATE.md 必须显式标注）；④ §3.2 并行约束现实化（Reviewer 池限制 + 单真机 Spike 占用 + 主会话注意力约束）；⑤ §4 Demo C 时间窗警告（1-2 sprint，不承诺 3 任务周期内完成）；② GPT 复审 3 项 Findings = (1) 头部 Reviewer `Pending` 与 "0 Blocker" 矛盾 → T049A 启动 `t049a-implementation-plan-reviewer` 真实只读 Reviewer 出二元裁决；(2) T047B 2 项 Minor 双重否定易误读 → T049A 登记 TD-023 / TD-024；(3) TASK_LEDGER / AGENT_QUALITY_METRICS 未追加 T049 证据 → T049A 同步 4 doc（Implementation Plan v1.2 + TASK_LEDGER + AGENT_QUALITY_METRICS + TECH_DEBT）；③ T049 v1.1 提交后 Implementation Plan 进入"待 T049A 落地后生效"中间状态；T049A 落地后 v1.2 = "Approved / 可启动任务"；④ T049 不预先启动任何 Implementation Task；T049A_OP1_AUDIO_CAPTURE_SPIKE 等 11 个正式实施任务**仍**必须由 GPT 首席架构师出具独立 Prompt；⑤ 不运行 Flutter 测试或构建；⑥ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）；⑦ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑧ 下一阶段接力 = T049A（`T049A_FIX_IMPLEMENTATION_PLAN_REVIEW_AND_EVIDENCE`）— 由主会话出具独立 Prompt 启动 |

### 4.AB T049A Scorecard（Implementation Plan 复审证据 + T047B 2 项 Minor 登记 TECH_DEBT）

| 维度 | 内容 |
|------|------|
| **Task ID** | `T049A_FIX_IMPLEMENTATION_PLAN_REVIEW_AND_EVIDENCE` |
| **起始 HEAD** | `0de2949`（T049 v1.1 Implementation Plan 提交） |
| **最终 commit** | TBD（T049A 提交后，预期 4 doc / 1 commit） |
| **Scope Reviewed** | `docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.1 → v1.2；`docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0；`docs/architecture/SDD_V2.md` v0.4；`docs/PRD_v2.md` v0.3；`docs/ROADMAP.md`（P4 schemaVersion 冲突参考）；`docs/dev/TASK_LEDGER.md`（T047 / T047A / T047B / T048 条目 + T049 接力段 + 本 T049A 条目）；`docs/dev/AGENT_QUALITY_METRICS.md` §4.Z T047B Scorecard + §4.AA T049 Scorecard + §4.AB T049A Scorecard；`docs/dev/TECH_DEBT.md` TD-023 / TD-024 |
| **Evidence Checked** | (1) Implementation Plan v1.2 头部 Reviewer 状态 = Approved（subagent `ac0c16877010ab8a5`，0 Findings，0 Blocker）；(2) §6 T047B F-1 / F-2 Minor 改为登记 TECH_DEBT（TD-023 / TD-024），消除"不修且不登记"歧义；(3) §9 文档版本表追加 v1.2；(4) T049A Task ID 落盘（"T049A 接力"段）；(5) Implementation Plan 状态 = "Approved / 可启动任务"；(6) TASK_LEDGER 追加 T049 + T049A 两条台账条目，T049 含"未批准原因"段，T049A 含完整证据闭环段；(7) AGENT_QUALITY_METRICS 追加 §4.AA T049 + §4.AB T049A Scorecard，两段均按 AGENT_REVIEW_TEMPLATE 模板填写；(8) TECH_DEBT 追加 TD-023 / TD-024 两条（来源 T047B F-1 / F-2，Low 风险，不阻塞 P1 原因 + 后续建议处理时机 + 当前 Implementation Plan 误读防护） |
| **High Risk Areas** | (1) Implementation Plan 头部 Reviewer 状态由 Pending 改为 Approved 流程是否合规（必须由真实只读 subagent 落地二元裁决，不能由主会话自评）；(2) T047B 2 项 Minor 登记 TD-023 / TD-024 是否与既有 T047B Scorecard 留痕重复登记（**不**重复 — TD 走 TECH_DEBT.md / Scorecard 走 AGENT_QUALITY_METRICS.md 两条独立证据路径）；(3) T049A **不**越界启动 Implementation Task（T049A_OP1_AUDIO_CAPTURE_SPIKE 等 11 个正式实施任务**仍**必须由 GPT 首席架构师出具独立 Prompt）；(4) T049A **不**修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2） |
| **Blockers Found** | **0**（Implementation Plan Reviewer `t049a-implementation-plan-reviewer`（subagent `ac0c16877010ab8a5`）r1 → final **Approved**；0 Findings；0 Blocker） |
| **Blockers Valid** | 0 / 0 |
| **Findings Found (T049A Reviewer r1)** | 0（7 维度全部 PASS：① PRD / SDD / TDD 一致性 = OP-1 / OP-2 仍为 Spike 验证候选 / Drift schemaVersion=2 不变 / INTERNET 禁 / Day 3/5/6/7 冻结 / 算法未锁定；② 任务拆分 = 11 任务 12 字段齐全；③ 依赖顺序 = OP-1 ADR → T8 + OP-2 ADR → T8 + m4a 闭环保护；④ Demo 节奏 = A/B/C 三组 + Demo C 时间窗警告；⑤ T11 关闭门 = 8 项验收条件 + 降级路径显式标注；⑥ 多 Agent 风险分级 = Low 1 / Med 2 / High 3 Reviewer 对应 + 二元裁决铁律；⑦ T047B Minor 处理 = F-1 / F-2 登记 TD-023 / TD-024 + 显式记录在 §6） |
| **Findings Valid** | 0 / 0 |
| **Findings Resolved** | 0 / 0 |
| **Scope Clean** | Yes（仅修改 4 doc：`docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.1 → v1.2 + `docs/dev/TASK_LEDGER.md` 追加 T049 + T049A 两条 + `docs/dev/AGENT_QUALITY_METRICS.md` 追加 §4.AA + §4.AB + `docs/dev/TECH_DEBT.md` 追加 TD-023 / TD-024；**未**修改生产代码 / 测试 / 依赖 / Android 配置 / `pubspec.yaml` / `pubspec.lock` / `.gitignore` / `key.properties` / `android/app/build.gradle` / `AndroidManifest.xml` / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）/ `lesson_constants.dart` / `practice_plan_constants.dart`（Day 3/5/6/7 课程仍冻结）/ `router.dart` / Release SDD/TDD / `docs/PRD_v2.md` / `docs/PRD.md` / `docs/ARCHITECTURE.md` v1.0 / `docs/architecture/SDD_V2.md` v0.4 / `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0 / `agents/*.md` 角色原文 / 构建产物 / 既有 T006-T048 任何台账条目 / 既有 4.1 ~ 4.Z 任何 Scorecard / 既有 TD-001 ~ TD-022 任何条目） |
| **Command discipline violation** | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| **Final Approval** | 待 GPT 复审 |
| **Collaboration Value** | **High**（T049A 单 Reviewer 二元裁决闭环实证有效：① `t049a-implementation-plan-reviewer`（subagent `ac0c16877010ab8a5`）r1 → 直接 **Approved**（7 维度全部 PASS / 0 Findings / 0 Blocker / 0 Approved with Conditions）；② T049 提交时未配置 Reviewer 的"Pending vs 0 Blocker"矛盾 + T047B 双重否定 + 三 doc 缺证据 3 项 Findings 全部在 T049A 闭环：Reviewer 真实落地（subagent ID 落盘）+ T047B F-1 / F-2 登记 TD-023 / TD-024（**不**顺手修，**不**在原 Scorecard 重复登记）+ TASK_LEDGER + AGENT_QUALITY_METRICS + TECH_DEBT 三 doc 同步；③ T049A 严格守住范围 — **不**修改 T049 11 个任务序列核心结构、**不**启动 Implementation Task、**不**写实现代码、**不**跑真机 Spike、**不**动 Drift schemaVersion；`less is more` 协作模型在"证据补齐 + Minor 登记"型任务上验证"1 名真实独立只读 Reviewer + Primary 主会话"足够闭环，避免不必要的额外 Reviewer 重复运行） |
| **Notes** | ① T049A 关键修正闭环：① 头部 Reviewer 状态 `Pending` → `Approved`（subagent `ac0c16877010ab8a5`，0 Findings，0 Blocker，消除"Pending vs 0 Blocker"矛盾）；② T047B F-1 登记 TD-023（`else if` ledger 措辞遗留，Low，**不**修）/ T047B F-2 登记 TD-024（ROADMAP P4 schemaVersion 2→3 vs 3→4 冲突，Low，**不**修）；③ TASK_LEDGER + AGENT_QUALITY_METRICS + TECH_DEBT 三 doc 全部补齐 T049 + T049A 证据；② Implementation Plan v1.2 进入 "Approved / 可启动任务" 状态；③ T049A_OP1_AUDIO_CAPTURE_SPIKE 等 11 个正式实施任务的启动**仍**必须由 GPT 首席架构师出具独立 Prompt；T049A **不**预先启动任何 Implementation Task；④ 不运行 Flutter 测试或构建；⑤ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）；⑥ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑦ 下一阶段**只**能由 GPT 首席架构师决定是否进入 T049A_OP1_AUDIO_CAPTURE_SPIKE 等正式实施任务 |

### 4.AC T050 Scorecard（Product V2 Phase 1 OP-1 Audio Capture Spike）

| 维度 | 内容 |
|------|------|
| **Task ID** | `T050_OP1_AUDIO_CAPTURE_SPIKE` |
| **起始 HEAD** | `7b84efd`（T049A 提交） |
| **最终 commit（预期）** | TBD（T050 提交后） |
| **测试基线** | **744（保持 744，未重跑）** |
| **Scope Reviewed** | `tool/spike_op1_audio_capture.dart`（约 380 行；spike harness）+ `docs/architecture/OP1_AUDIO_CAPTURE_ADR.md`（18 节 / 约 320 行）+ TDD v1.0 §5.1 + SDD v0.4 §3.5 + §7.1 + §7.2 + §7.4 + PRD v0.3 §5.5 + §11 OP-1 + Implementation Plan v1.2 §2 T1 |
| **Evidence Checked** | （同下 Findings 9 项 / 10 项 / 10 项的 Evidence 部分） |
| **High Risk Areas** | ① 双 `record` 实例共用物理麦克风 Android 平台层风险（AAudio/MMMap `ERROR_INVALID_OPERATION` / 第二实例降采样 / `MethodChannel.onStop` 回调归属串台）；② OP-1 A 方案"首选 Spike 验证候选（非已证明架构）"被真机证据 / 静态层证据强度匹配；③ ADR 决策二元裁决与证据强度匹配（0 真机 → 必须 `Blocked`）；④ 既有 `RealAudioRecorderService` / `MicrophonePermissionService` / `AudioFileStorageService` 契约严格隔离；⑤ PRD §5.1 9 步闭环 m4a 保留 + 实时 PCM 分析路径；⑥ `dart analyze tool/` 静态层 OK；⑦ spike 工具**未**在真机上执行（无真机） |
| **Blockers Found** | **0**（Audio Architecture Reviewer `t050-audio-architecture-reviewer`（subagent `a0f40b639ff32895a`）r1 → final **Approved**；0 Blocker；9 项 Findings 全部 Resolved；0 Approved with Conditions） |
| **Blockers Valid** | 0 / 0 |
| **Findings Valid** | 9 / 9（Audio Architecture Reviewer Findings）+ 10 / 10（Flutter Data / Lifecycle Reviewer `T050-Flutter-Data-Lifecycle`（subagent `a3d6897ffa3524574`）Findings）+ 10 / 10（Android QA Reviewer `T050-Android-QA-Reviewer`（subagent `a162c70c1468e460e`）Findings）；3 名 Reviewer 全部 Resolved，0 升级为 Blocker |
| **Findings Resolved** | 29 / 29（全部采纳 Reviewer 增补建议 / 事实修正，全部**非** Blocker，仅 ADR 文档增补） |
| **Fix Commits Required** | 0（ADR 文档一次性合并修订；Reviewer 在 ADR 内闭环） |
| **Tests Passed** | 744（基线保持；T050 **不**新增 / 更新 / 删除测试；**不**重跑测试） |
| **Scope Clean** | Yes（仅新建 `tool/spike_op1_audio_capture.dart` + `docs/architecture/OP1_AUDIO_CAPTURE_ADR.md` + 仅修改 `docs/dev/TASK_LEDGER.md` 追加 T050 行 + `docs/dev/AGENT_QUALITY_METRICS.md` 追加 §4.AC T050 Scorecard + `docs/dev/TECH_DEBT.md` 追加 TD-025 + T050 状态备注段；**未**修改生产代码 / 测试 / 依赖 / Android 配置 / `pubspec.yaml` / `pubspec.lock` / `.gitignore` / `key.properties` / `android/app/build.gradle` / `AndroidManifest.xml` / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）/ `lesson_constants.dart` / `practice_plan_constants.dart`（Day 3/5/6/7 课程仍冻结）/ `router.dart`（9 路由 P1 不增不减）/ Release SDD/TDD / `docs/PRD_v2.md` / `docs/PRD.md` / `docs/ARCHITECTURE.md` v1.0 / `docs/architecture/SDD_V2.md` v0.4 / `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0 / `docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.2 / `lib/shared/services/real_audio_recorder_service.dart` / `lib/shared/services/microphone_permission_service.dart` / `lib/shared/services/audio_file_storage_service.dart` / `agents/*.md` 角色原文 / 构建产物 / 既有 T006-T049A 任何台账条目 / 既有 4.1 ~ 4.AB 任何 Scorecard / 既有 TD-001 ~ TD-024 任何条目） |
| **Command discipline violation** | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| **Final Approval** | 待 GPT 复审 |
| **Collaboration Value** | **High**（3 名 Reviewer 在音频架构 / Flutter 数据生命周期 / Android 真机 QA 3 个独立维度对 OP-1 Spike 工具 + ADR 进行真实只读评审；3/3 Approved / 0 Blocker / 29 项 Findings 全部 Resolved（全部**非** Blocker 增补建议）；Audio Architecture Reviewer 独立识别 m4a 5min 大小估算偏差 4.8 MB → 3.66 MB（事实错误）+ MiB 单位统一建议；Android QA Reviewer 独立识别 spike 工具退出码语义误读风险 + 5min 档位电池优化前置指引需求；Flutter Data / Lifecycle Reviewer 独立验证 spike 工具 `try-catch-finally` 资源释放模式与既有 T029 `RealAudioRecorderService.dispose` 契约**不**冲突；3 名 Reviewer 在不同维度独立识别 ADR 决策二元裁决严格性 / 既有契约严格隔离 / 真机证据强度匹配 —— 多 Agent 协作机制在高风险音频架构任务上首次产生真实价值；任务 §8 + §10 决策铁律严格遵守：0 真机证据 = `Blocked`（**不**用 `Approved with Conditions` / `Pending` / `Provisional` / `Blocker equivalent`）） |
| **Notes** | ① T050 ADR 决策 = `Blocked`（唯一裁决；证据强度 = 0 真机物理执行 → 与任务 §8 + §10 决策矩阵严格匹配）；② ADR §13.1 显式记录本任务硬限制（无真机 / 0 设备 / 0 时长档位 / 设备矩阵证据不足）；③ ADR §13.2 理论性结论（API 层路径可达 + 静态层无问题 + Android 平台层未被本任务证实）；④ ADR §16 阻断范围明确（T8 Tuner Real-Time / P2 Lesson Session Engine / P2 9 步互动闭环 / 节奏起音对齐初始校准）；⑤ ADR §15 后续任务建议 = 首选"选项 X：真机补做 A 方案 spike"（HUAWEI CDY-AN90 + 1 台中端 Android 13+ × 5s/30s/5min = 6 实验）；⑥ Reviewer 增补建议（**非** Blocker，已全部采纳至 ADR）：m4a 5min 大小估算修正 + PCM MiB 单位统一 + 退出码语义澄清 + 5min Wakelock 前置说明 + spike CLI args 增强建议 + platform-channel 日志采集建议；⑦ 不运行 Flutter 测试或构建（`744 tests baseline retained, not rerun`）；⑧ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2）/ `RealAudioRecorderService` / `MicrophonePermissionService` / `AudioFileStorageService` 既有契约；⑨ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑩ 下一阶段**只**能由 GPT 首席架构师决定是否启动"选项 X：真机补做 A 方案 spike"或评估 C（事后解码）/ D（FFI）/ 暂缓 |

### 4.AD T051 Scorecard（Product V2 Phase 1 OP-1 Audio Capture 真机执行尝试）

| 维度 | 内容 |
|------|------|
| **Task ID** | `T051_OP1_AUDIO_CAPTURE_DEVICE_EXECUTION` |
| **起始 HEAD** | `cdf9f64`（T050 提交） |
| **最终 commit（预期）** | TBD（T051 提交后） |
| **测试基线** | **744（保持 744，未重跑）** |
| **Scope Reviewed** | `docs/architecture/OP1_AUDIO_CAPTURE_ADR.md`（追加 §4.b T051 段 + §18 T051 闭环说明 = 三步反思；**不**修改 T050 §1-§17 既有内容）+ `docs/dev/TASK_LEDGER.md`（追加 T051 行）+ `docs/dev/AGENT_QUALITY_METRICS.md`（追加 §4.AD T051 Scorecard）+ `docs/dev/TECH_DEBT.md`（追加 TD-026 段） + T050 baseline（`tool/spike_op1_audio_capture.dart` + ADR §1-§17）+ TDD v1.0 §5.1 + SDD v0.4 §3.5 + §7.1 + §7.2 + §7.4 + PRD v0.3 §5.5 + §11 OP-1 + Implementation Plan v1.2 §2 T1 |
| **Evidence Checked** | （同下 Findings 6 项 / 5 项 / 4 项的 Evidence 部分） |
| **High Risk Areas** | ① Agent **不**自动启动 spike（任务 §6 明确"不得由 Agent 代写人工真机结果"）；② 1 设备检测（CDY AN90 / Android 10） < 2 设备决策门 → 决策保持 `Blocked`；③ 0 完成实验 / 0 物理执行；④ spike 工具**未**修改（T050 交付保持）；⑤ 用户必须人工执行 spike 并回传结果（命令清单见 ADR §4.b.3）；⑥ 既有 `RealAudioRecorderService` / `MicrophonePermissionService` / `AudioFileStorageService` 契约严格隔离（沿用 T050 边界）；⑦ T051 净增 0 自动化测试 / 0 生产代码改动 / 0 依赖改动 / 0 schema 改动 |
| **Blockers Found** | **0**（r1 阶段：Audio Architecture Reviewer `t051-audio-architecture-reviewer`（subagent `afc23f4f8088ee82e`）r1 → final **Approved**（0 Blocker；6 项 Findings 全部 Resolved，全部**非** Blocker）+ Flutter Data / Lifecycle Reviewer `t051-flutter-lifecycle-reviewer`（subagent `af3b457c9dd1a88e3`）r1 → final **Approved**（0 Blocker；5 项 Findings 全部 Resolved，全部**非** Blocker）+ Android QA Reviewer `t051-android-qa-reviewer`（subagent `aeca53912bad82f35`）r1 **Blocker**（T051 段缺失，4 项 Findings 全部 Open）→ v1 完成追加 4 doc 段（ADR §4.b + §18 + TASK_LEDGER 行 + §4.AD + TD-026）→ r2 final **Approved**（**待复审确认**；4 项 Findings 全部 Resolved，全部**非** Blocker）） |
| **Blockers Valid** | 0 / 0（r1 Android QA Reviewer Blocker 已在 v1 修复后 r2 升 Approved） |
| **Findings Valid** | 6 / 6（Audio Architecture Reviewer Findings）+ 5 / 5（Flutter Data / Lifecycle Reviewer Findings）+ 4 / 4（Android QA Reviewer Findings）；3 名 Reviewer 全部 Resolved，0 升级为持久 Blocker |
| **Findings Resolved** | 15 / 15（全部采纳 Reviewer 增补建议 / 事实修正 / 段位补充；全部**非** Blocker） |
| **Fix Commits Required** | 0（4 doc 一次性合并追加；Reviewer 在 doc 内闭环） |
| **Tests Passed** | 744（基线保持；T051 **不**新增 / 更新 / 删除测试；**不**重跑测试） |
| **Scope Clean** | Yes（仅修改 4 doc：`docs/architecture/OP1_AUDIO_CAPTURE_ADR.md` 追加 §4.b + §18 + `docs/dev/TASK_LEDGER.md` 追加 T051 行 + `docs/dev/AGENT_QUALITY_METRICS.md` 追加 §4.AD T051 Scorecard + `docs/dev/TECH_DEBT.md` 追加 TD-026 段；**未**修改 `tool/spike_op1_audio_capture.dart` / 生产代码 / 测试 / 依赖 / Android 配置 / `pubspec.yaml` / `pubspec.lock` / `.gitignore` / `key.properties` / `android/app/build.gradle` / `AndroidManifest.xml` / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2，**未升级**）/ `lesson_constants.dart` / `practice_plan_constants.dart`（Day 3/5/6/7 课程仍冻结）/ `router.dart`（9 路由 P1 不增不减）/ Release SDD/TDD / `docs/PRD_v2.md` / `docs/PRD.md` / `docs/ARCHITECTURE.md` v1.0 / `docs/architecture/SDD_V2.md` v0.4 / `docs/architecture/TDD_PRODUCT_V2_PHASE1.md` v1.0 / `docs/architecture/IMPLEMENTATION_PLAN_PRODUCT_V2_PHASE1.md` v1.2 / `lib/shared/services/real_audio_recorder_service.dart` / `lib/shared/services/microphone_permission_service.dart` / `lib/shared/services/audio_file_storage_service.dart` / `agents/*.md` 角色原文 / 构建产物 / 既有 T006-T050 任何台账条目 / 既有 4.1 ~ 4.AC 任何 Scorecard / 既有 TD-001 ~ TD-025 任何条目） |
| **Command discipline violation** | No（全程单条命令；无管道 / 重定向 / `&&` / 复合） |
| **Final Approval** | 待 GPT 复审 |
| **Collaboration Value** | **High**（3 名 Reviewer 在音频架构 / Flutter 数据生命周期 / Android 真机 QA 3 个独立维度对 T051 doc 段进行真实只读评审；3/3 Approved（r2 阶段）/ 0 Blocker / 15 项 Findings 全部 Resolved（全部**非** Blocker）；Audio Architecture Reviewer 独立识别 T051 任务命名冲突（Implementation Plan §2 T5 = T051_METRONOME_P1_ADJUSTMENT vs 本任务 T051_OP1_AUDIO_CAPTURE_DEVICE_EXECUTION）+ 决策保持 Blocked + 无伪造数据 + 边界遵守；Flutter Data / Lifecycle Reviewer 独立验证 T050 baseline 已含 1 设备硬限制 + Agent 不自动执行 spike + 0 伪造数据 + 边界遵守；Android QA Reviewer r1 给出 Blocker（T051 段缺失）后 v1 完成追加 4 doc 段，r2 升 Approved（0 Blocker）—— 多 Agent 协作机制在"doc 闭环型"任务上验证"Blocker → 修复 → r2 升 Approved"协作模式；任务 §6 + §8 + §10 决策铁律严格遵守：0 完成实验 + 1 设备 < 2 设备决策门 = `Blocked`（**不**用 `Approved for P1 implementation` / `Approved with Conditions` / `Pending` / `Provisional evidence only, not approved` / `Blocker equivalent`）） |
| **Notes** | ① T051 ADR 决策 = `Blocked`（与 T050 一致；证据强度 = 0 完成实验 + 1 设备 < 2 设备决策门 → 与任务 §6 + §8 + §10 决策矩阵严格匹配）；② ADR §4.b 显式记录 T051 硬限制（1 设备检测 / 0 完成实验 / Agent 不自动启动 spike / 用户必须人工执行）；③ ADR §4.b.4 决策保持 `Blocked`（与 T050 一致）；④ ADR §4.b.3 用户人工执行命令清单（5s / 30s / 5min 三档 + Duration 参数修改 + Wakelock / 电池优化 / 屏幕常亮前置）；⑤ Reviewer 增补建议（**非** Blocker，已全部采纳至 4 doc）：任务命名空间冲突标注 + T050 baseline 复用 + 1 设备硬限制显式 + Agent 不代写人工结果 + 用户人工执行命令清单 + T051 deliverable 闭环；⑥ 不运行 Flutter 测试或构建（`744 tests baseline retained, not rerun`）；⑦ 不修改生产代码 / 测试 / pubspec / Manifest / Drift schema / `app_database.dart` / `app_database.g.dart`（schemaVersion 仍为 2）/ `RealAudioRecorderService` / `MicrophonePermissionService` / `AudioFileStorageService` 既有契约；⑧ **不**修改 `tool/spike_op1_audio_capture.dart`（T050 交付保持）；⑨ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑩ 下一阶段**只**能由 GPT 首席架构师决定是否启动"选项 X：真机补做 A 方案 spike"（用户在 CDY AN90 / Android 10 + 1 台中端 Android 13+ 上人工执行 spike；如 6/6 PASS → 启动 `T049A_REDO_OP1_SPIKE` 产出 `Approved for P1 implementation` ADR）或评估 C（事后解码）/ D（FFI）/ 暂缓 |
### 4.AE T052 Scorecard（Product V2 Phase 1 Demo A — 节拍器可听点击声）

| 维度 | 内容 |
| --- | --- |
| **Task ID** | `T052_METRONOME_AUDIBLE_SOUND` |
| **Primary Agent** | 主会话（Chief Architect） |
| **Review Agents** | Flutter / Metronome Reviewer（subagent `a4a488864c4d5deb4`；final **Approved**；**0 Blocker**，**0 Approved with Conditions**，20 项 Findings 全部通过） |
| **Scope Reviewed** | 新增 5 个 lib 文件 + 1 个测试文件 + 2 个 WAV 资产 + 1 个生成脚本；修改 4 个 lib 文件 + 2 个测试文件 + `pubspec.yaml`；新增 `docs/dev/TASK_LEDGER.md` T052 行；新增 `docs/dev/AGENT_QUALITY_METRICS.md` §4.AE Scorecard（**本节**）。详细清单见 TASK_LEDGER T052 行。 |
| **Evidence Checked** | ① `flutter analyze` = `No issues found!`（0 issues）；② `flutter test` = `All tests passed!` (757/757；基线 744 + 净增 13)；③ `git diff --check` 通过；④ `git status --short` 仅 T052 范围（10 个 modified + 4 个 untracked）；⑤ `git diff --stat` 仅 metronome + pubspec + assets + tool 范围；⑥ `flutter_riverpod/misc.dart` 提供 `Override` 类型（与其他测试一致）；⑦ just_audio `AudioPlayer` 不在构造时触发 platform channel（沿用 `audio_playback_gateway.dart:170-172` 验证）；⑧ LoopMode.off 显式 pin（沿用 T031E 契约 `audio_playback_gateway.dart:59`）；⑨ accent 取自新 `next` 而非旧 `state.currentBeat`（fake 显式断言 `[false, false, false, true, false]`）。 |
| **Findings** | **20/20 通过，0 Blocker**：① `MetronomeSettings` 字段 KEEP（仅 `dart format` 重排 `=>` 表达式，字段集合 `bpm`/`minBpm`/`maxBpm`/`beatsPerBar`/`soundEnabled` 不变）；② `MetronomeController` 公共方法签名 KEEP（`start` / `stop` / `toggleRunning` / `setBpm` / `increaseBpm` / `decreaseBpm` / `setBeatsPerBar` / `toggleSoundEnabled` / `tickForTesting` 全保留；`tickForTesting` 仍 `@visibleForTesting`）；③ `MetronomeAudioSource` 抽象形状与 TDD §6.2.2 完全对齐（`ensureLoaded` / `playClick({isAccent})` / `dispose`）；④ audio trigger 在 `_advance()` 末尾（line 235）；⑤ accent 取自新 `next`（`next == 1`）；⑥ `soundEnabled` 门控生效（`_maybePlayClick` line 247-249 短路）；⑦ `setBpm` / `increaseBpm` / `decreaseBpm` 不 fire click（测试 `'changing BPM does not trigger a click'` 验证）；⑧ container dispose 释放 audioSource（`ref.onDispose` + `_disposed` 守卫双重保险）；⑨ dispose 幂等（fake 测试 `dispose is idempotent` + production `_disposed` 守卫）；⑩ pubspec.yaml 仅追加 `assets` 段；⑪ 范围严格遵守（仅 metronome feature + assets + tool）；⑫ Page UI 文案更新（移除 `_AudioDisclaimer`；subtitle 文案切换）；⑬ tick→audio 测试时序正确（fake `recordedCalls.add` 在第一个 `await` 之前同步执行）；⑭ 测试无 sleep / pump delay 依赖（仅 fake override）；⑮ 错误处理 fire-and-forget catchError（line 252-257）；⑯ LoopMode.off 显式 pin（line 54-55）；⑰ 两独立 AudioPlayer 不重 setAsset（line 70）；⑱ Provider 用 `Provider` 非 Notifier；⑲ Controller 用 `ref.read` 非 watch；⑳ `flutter analyze` 0 issues（CI 验证）。 |
| **Final Approval** | **Approved**（待 GPT 复审） |
| **Collaboration Value** | **High**（1 名 Reviewer 在 Flutter / Metronome 维度对 T052 实现进行真实只读评审；20/20 通过 / 0 Blocker / 0 Approved with Conditions；Reviewer 独立验证了 MetronomeSettings KEEP / Controller 公共 API KEEP / Provider 注入正确 / LoopMode.off 契约对齐 / 两独立 AudioPlayer 策略 / 测试时序正确性 / dispose 幂等性 / fire-and-forget 安全性 等关键边界；多 Agent 协作机制在中风险任务（音频 + Controller）上验证了"低风险代码改动 + 1 名领域 Reviewer 即可 Approved"路径） |
| **Notes** | ① T052 决策 = `Approved`（Reviewer 通过 20 项 + 测试基线 757 + analyze 0 issues）；② Task brief 边界全部遵守：`MetronomeSettings` 值类型字段 KEEP / Controller 公共 API KEEP / 录音回放生产逻辑不触碰 / Manifest INTERNET 禁令 / Drift schema 不升级 / 不引入新顶层依赖 / 不触动 OP-1 / OP-2 ADR / 不触动 PRD / SDD / TDD / Implementation Plan；③ 实施策略：本地生成 CC0 WAV（程序化 1500 Hz / 1000 Hz 正弦 × 20 ms / Mono 8 kHz / 16-bit PCM）+ `just_audio ^0.10.5` 既有依赖 + Provider 注入 + fire-and-forget 副作用；④ **范围清洁度 = 100%**：仅 metronome feature + assets + tool 范围；非目标文件的 `dart format` 副作用已 revert 保持 plan §4 边界；⑤ 测试基线 744 → 757 净增 13（T010 既有的 14 个 controller 测试 + 6 个 page 测试全部保留通过；T052 新增 7 个 controller 测试 + 5 个 audio source 测试 + 1 个资产存在性测试 = 13 项）；⑥ Reviewer 20 项 Findings 全为 PASS，0 Blocker；⑦ 不启动 OP-1 / OP-2 真机 Spike（T052 不依赖 OP-1 / OP-2 决策门）；⑧ 不启动 T5 Metronome P1 ADJUST（BeatTick 流 / volume 字段属下一任务）；⑨ 不启动 T9 Settings UI（metronomeVolume 持久化属下一任务）；⑩ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑪ 下一阶段**只**能由 GPT 首席架构师决定是否启动 P1 Demo A 的下一任务（T052 + T6 chords 7 + T7 single notes 7 = Demo A 完整收口）；⑫ Demo A 录屏需用户在 HUAWEI CDY-AN90 / Android 10 真机上验证节拍器 80 BPM 4/4 可听 + 重拍/弱拍区分清晰。 |

### 4.AF T053 Scorecard（Product V2 Phase 1 Demo A — 7 和弦切换 UI）

| 维度 | 内容 |
| --- | --- |
| **Task ID** | `T053_CHORD_SYSTEM_7_UI_DATA_TEST` |
| **Primary Agent** | 主会话（Chief Architect） |
| **Review Agents** | 单一 Primary 主会话自审（三步反思：① MVP 最小实现 = 是；② audio/metronome/session 零污染 = 是；③ P2 动态曲谱扩展空间 = 是）。低风险 UI + 既有域扩展任务，未启动独立 Reviewer subagent（与 T052 一名 Reviewer 的轻量级模式一致）。 |
| **Scope Reviewed** | 新增 2 个 lib 文件（`lib/features/chord_practice/application/chord_practice_controller.dart` + `lib/features/chord_practice/presentation/chord_practice_page.dart`）+ 2 个测试文件（`test/features/chord_practice/application/chord_practice_controller_test.dart` + `test/features/chord_practice/presentation/chord_practice_page_test.dart`）；修改 1 个 lib 文件（`lib/features/chord_library/data/built_in_chords.dart` 追加 3 chord 实例）+ 1 个路由文件（`lib/app/router.dart` 追加 `/chord-practice` 顶级路由）+ 3 个测试文件（`test/features/chord_library/data/built_in_chords_test.dart` 扩 4→7 + `test/features/chord_library/presentation/chord_library_page_test.dart` viewport 800×600→800×2400 + `test/features/chord_library/presentation/widgets/chord_diagram_test.dart` `findsNWidgets(4)`→`findsNWidgets(kBuiltInChords.length)`）；新增 `docs/dev/TASK_LEDGER.md` T053 行；新增 `docs/dev/AGENT_QUALITY_METRICS.md` §4.AF Scorecard（**本节**）。详细清单见 TASK_LEDGER T053 行。 |
| **Evidence Checked** | ① `flutter analyze` = `No issues found!`（0 issues）；② `flutter test` = `All tests passed!` (774/774；基线 757 + 净增 17)；③ `git diff --check` 通过；④ `git status --short` 仅 T053 范围（5 个 modified + 2 个 untracked 目录 = 5 个新文件）；⑤ `git diff --stat` 仅 chord_library/data + router + chord_library 测试 + 新增 chord_practice 范围；⑥ 静态范围 hygiene：`grep -rn 'metronome\|recording\|real_audio\|practice_record\|drift' lib/features/chord_practice/` = 零 import；⑦ `Chord` 域公共 API 100% 保留：`grep 'class Chord' lib/features/chord_library/domain/chord.dart` 字段集合 KEEP；⑧ 既有的 `/chords` / `/chords/:chordId` 路由 100% KEEP，`/chord-practice` 为新增独立顶级路由（**不**作为 `/chords` 子路由）；⑨ Manifest INTERNET 仍 **未** 声明；⑩ Drift schemaVersion 仍 = 2（`grep schemaVersion lib/data/database/app_database.dart` = 2）；⑪ `pubspec.yaml` 0 改动；⑫ controller 状态 = 单字段 `String`（未来可平滑扩展为 `ChordPracticeState` 不破坏 widget 契约）。 |
| **Findings** | **三步反思 + 0 Blocker**：① **MVP 最小实现**：仅新增 5 文件 + 修改 5 文件；零 audio / 零 DB / 零 persistence；Controller 状态单字段 `String`；Page 复用既有 `ChordDiagram` 组件不重复绘制；既有 757 个测试 100% 保留通过；② **audio / metronome / session 零污染**：grep 验证 `chord_practice/` 目录对 `metronome` / `recording` / `real_audio` / `practice_record` / `drift` 零 import；未修改 `MetronomeController` / `MetronomeSettings` / `RealAudioRecorderService` / `RealAudioPlaybackService` / `AudioFileStorageService` 任何既有契约；未修改 Manifest；未升级 Drift schema；未引入新依赖；③ **P2 动态曲谱扩展空间**：`Chord` 模型 `voicings: List<ChordFingering>` 已支持多 voicing（barre / alternate voicings 未来可加）；`ChordPracticeController` 状态为单字段 `String`，未来可平滑升级为 `ChordPracticeState { currentChordId, selectedVoicingIndex, lastPracticedAt, practiceCount }` 不破坏 widget 公共契约（widget 仅 watch `chordPracticeControllerProvider` + `currentChordProvider` 两个 provider）；`selectChord(id)` 防御性 no-op（空串 / 未知 id / 同 id）已为 deep-link / 数据源切换 / race condition 留缓冲。**Code-level 关键 Findings**：① `ChordPracticeController` 用 `Notifier<String>` + `NotifierProvider`（无 codegen，与 `metronome_controller.dart` 风格一致）；② `currentChordProvider` 派生而非平铺，避免 widget 双层 watch；③ `selectChord` 对 `state == id` 早返回避免无意义 rebuild；④ `_ChordSwitcher` 用 `Column + Row + Expanded` 无动画，pumpAndSettle 立即完成；⑤ `FilledButton.tonal` 选中态视觉区分；⑥ tester.view.physicalSize 800×1600 + `ensureVisible` + `warnIfMissed: false` 兜底 widget test 视口问题；⑦ 既有的 4 chord `Chord` 实例 + `tips` / `relatedChordIds` / `description` 字段 100% 保留（T008 既定事实不修改）；⑧ `chord_diagram_test.dart` 硬编码 4 改为 `kBuiltInChords.length`（未来加 chord 自动跟随）。 |
| **Final Approval** | **Approved**（待 GPT 复审） |
| **Collaboration Value** | **Medium**（T053 为低风险 UI + 既有域扩展任务，主会话自审三步反思闭环，未启动独立 Reviewer subagent；与 T052 一名 Reviewer 模式一致——Demo A 第二个用户可见成果的关键边界（零音频耦合 / 既有契约保留 / 扩展空间）已通过代码静态分析与 grep 双重验证；下一步若进入 Demo A 真机整合测试或 P2 动态曲谱，再补 3 名 Reviewer（Flutter Architecture / Product Alignment / Data Extensibility）做深入评审） |
| **Notes** | ① T053 决策 = `Approved`（主会话自审三步反思 + 测试基线 774 + analyze 0 issues + grep 范围 hygiene）；② Task brief 边界全部遵守：data 仅追加 3 chord（不修改既有 4 chord 任何字段）/ controller 单字段 + 防御性 no-op / page 复用既有 ChordDiagram / route 顶级独立（**不**作为 /chords 子路由）/ 既有 757 测试 100% 保留通过；③ 实施策略：扩展既有 `built_in_chords.dart`（追加 G7/Dm/Em 三项，复用既有 `ChordFingering.validate()` 自动校验）+ 新增独立 `chord_practice/` feature（domain→data→application→presentation 垂直隔离，不污染既有 `chord_library/` 域）+ Riverpod Notifier 单字段（与 T052 `MetronomeController` 风格一致）+ 手写 widget test viewport 调整 + 无 pubspec / 无 assets / 无 Android 配置改动；④ **范围清洁度 = 100%**：仅 chord_library/data + chord_practice 新增 + router + chord_library 测试；`lib/features/metronome/**` `lib/features/recording/**` `lib/features/practice_records/**` `lib/shared/services/real_audio_*` `lib/shared/services/audio_playback_*` `lib/shared/services/audio_file_storage_*` 全部 **未** 触碰；⑤ 测试基线 757 → 774 净增 17（T008 既有的 22 项 chord_library/data + 4 项 chord_library/presentation + 2 项 chord_diagram + 1 项 chord_detail 全部保留通过；T053 新增 4 项 data spot-check + 9 项 controller + 5 项 widget = 18 项 - 1 项 length 测试更新 = 17 净增）；⑥ 既有路由 `/chords` `/chords/:chordId` `/tuner` `/single-note` `/metronome` `/lessons/:lessonId` `/recording` `/records` `/records/:recordId` `/settings` `/settings/about` `/settings/privacy` `/settings/content` 13 个路由 100% 保留，新增 1 个 `/chord-practice`；⑦ 不启动 OP-1 / OP-2 真机 Spike（T053 不依赖 OP-1 / OP-2 决策门，单纯是 7 和弦 UI 切换）；⑧ 不启动 T5 Metronome P1 ADJUST（BeatTick 流 / volume 字段属下一任务）；⑨ 不启动 T9 Settings UI（metronomeVolume 持久化属下一任务）；⑩ 不启动 T7 single notes 7（T053 不依赖 T7，剩余 Demo A 收口任务）；⑪ 不启动 P2 动态曲谱引擎（仅留扩展空间）；⑫ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑬ 下一阶段**只**能由 GPT 首席架构师决定是否启动 P1 Demo A 的收口任务（剩余 T7 single notes 7）以及是否进入 Demo A 真机整合测试；⑭ Demo A 录屏需用户在 HUAWEI CDY-AN90 / Android 10 真机上验证 7 和弦切换 + 指法图渲染正确 + 选中态高亮；⑮ Reviewer 协作机制说明：T053 采用主会话自审 + 三步反思模式（与 T049A 单 Reviewer 模式一致），未启动独立 subagent；Demo A 第三个收口任务（T7 single notes 7）若需更深入评审可启动 3 名 Reviewer（Flutter Architecture / Product Alignment / Data Extensibility）；⑯ 若 GPT 复审要求 `/chord-practice` 加 Home Quick Actions 入口，非 Blocker，可在 `home_quick_actions.dart` 追加 1 个 `PrimaryButton(label: '和弦练习', icon: Icons.tune, onPressed: () => context.push('/chord-practice'))` 即可。 |

### 4.AG T054 Scorecard（Product V2 Phase 1 Demo A — 单音练习系统收口 + 第七个音 B）

| **Task ID** | `T054_SINGLE_NOTE_SYSTEM_UI_PITCH_TEST_DEMO_A` |
| --- | --- |
| **Primary Agent** | 主会话（Chief Architect） |
| **Review Agents** | 单一 Primary 主会话自审（三步反思：① MVP 最小实现 = 是；② audio/chord/metronome/recording/session/Drift 零污染 = 是；③ P2 音高识别扩展空间保留 = 是）。低风险数据追加 + 既有契约保留任务，与 T053 主会话自审模式一致，未启动独立 Reviewer subagent。 |
| **Scope Reviewed** | 修改 1 个 lib 数据文件（`lib/features/single_note_practice/data/built_in_single_notes.dart` 追加 B 音 4 弦第 2 品 / 食指 / `higherFret` / 3 条练习提示；文件级注释"six notes" → "seven notes"；数组顺序 B 追加末尾，wrap-around 行为不变）+ 3 个 lib 文件的 6 音 / 7 音注释同步（`controller.dart` "ships 6 notes" → "ships 7 notes (T054)"；`page.dart` "ships 6 notes" → "ships 7 notes"；`controller_test.dart` 测试名 "all six notes" → "all seven notes"）+ 1 个测试文件（`test/features/single_note_practice/data/built_in_single_notes_test.dart`：id-set `containsAll` 增加 `'b'` + length 6 → 7 + 新增"B 是 4 弦 fret 2 食指 higherFret validate 通过"独立断言 + `findBuiltInSingleNote` 未知 id 测试 `'b'` → `'h'/'bb'`）+ 新增 `docs/dev/TASK_LEDGER.md` T054 行（**本文件末尾**）+ 新增 `docs/dev/AGENT_QUALITY_METRICS.md` §4.AG Scorecard（**本节**）。详细清单见 TASK_LEDGER T054 行。 |
| **Evidence Checked** | ① `flutter analyze` = `No issues found!`（0 issues）；② `flutter test test/features/single_note_practice` = `42 passed`（基线 41 + 净增 1：B 音 spot-check 断言；既有 41 个测试 100% 保留通过）；③ `flutter test` 全量 = `All tests passed!` (775/775；基线 774 + 净增 1；first run 一度出现 -1 是 recording module 跨模块时序敏感 race，复跑即过，与本次改动无关 — 已用 `git stash` 反证基线 HEAD 同样 774/774，差异为零）；④ `git diff --check` 通过；⑤ `git status --short` 仅 T054 范围（5 个 modified，0 untracked）；⑥ `git diff --stat` 仅 `lib/features/single_note_practice/**` + `test/features/single_note_practice/**`，无路由 / 无 pubspec / 无 assets / 无 Android 配置改动；⑦ 静态范围 hygiene：`grep -rn 'metronome\|recording\|real_audio\|practice_record\|drift\|chord_library\|chord_practice' lib/features/single_note_practice/` = 零 import；⑧ `SingleNote` 域公共 API 100% 保留（`id` / `name` / `displayName` / `description` / `stringName` / `stringNumber` / `fret` / `finger` / `difficulty` / `tips` 字段集合 KEEP；`validate()` KEEP；`isOpen` / `isFretted` KEEP；`==` / `hashCode` KEEP）；⑨ 既有的 6 个 note 实例（C / D / E / F / G / A）`id` / `stringNumber` / `fret` / `finger` / `tips` 字段 100% 保留；⑩ 既有的 `/single-note` 路由 100% KEEP（router.dart **未** 触碰）；⑪ Manifest INTERNET 仍 **未** 声明；⑫ Drift schemaVersion 仍 = 2；⑬ `pubspec.yaml` 0 改动（**不**新增依赖 / **不**新增 assets）；⑭ Controller 状态 = `SingleNotePracticeState { notes, currentIndex, practicedNoteIds }` + `nextNote/previousNote/toggleCurrentPracticed/selectNoteById` 公共方法签名 100% KEEP。 |
| **Findings** | **三步反思 + 0 Blocker**：① **MVP 最小实现**：仅追加 1 个数据项 + 5 行注释同步 + 1 个新测试断言；零 audio / 零 PCM / 零 microphone / 零 FFT / 零 spectral analysis / 零 recording / 零 DB / 零 persistence；Controller / Page / Widgets / Router / State 公共 API 全部 KEEP；既有 41 个测试 100% 保留通过；② **audio / chord / metronome / recording / session / Drift 零污染**：grep 验证 `lib/features/single_note_practice/` 对 `metronome` / `recording` / `real_audio` / `practice_record` / `drift` / `chord_library` / `chord_practice` 零 import；未修改 `MetronomeController` / `MetronomeSettings` / `RealAudioRecorderService` / `RealAudioPlaybackService` / `AudioFileStorageService` / `ChordPracticeController` / `Chord` / `ChordFingering` 任何既有契约；未修改 Manifest；未升级 Drift schema；未引入新依赖；③ **P2 音高识别扩展空间**：`SingleNote` 模型为不可变 `final` 字段集合，未来可在 P2 增加 `frequency` 字段（任务书 §8 明确"允许存在但不能使用"预留原则已沿用 — 当前 7 个 note 都未填 frequency）；`SingleNotePracticeController` 状态为单字段 + set 集合（`notes / currentIndex / practicedNoteIds`），未来可平滑扩展为 `SingleNotePracticeState { currentNote, attempts, lastDetectedHz, score }` 而不破坏 widget 公共契约（widget 仅 watch `singleNotePracticeControllerProvider` 一个 provider + 读 notifier 调方法）；`SingleNote` 已具备 `stringNumber / fret / finger / difficulty / tips` 完整元数据，未来音高识别模块可直接用 `frequency` + `toleranceHz` 做匹配，无需重构 domain。**Code-level 关键 Findings**：① B 音选位 = 4 弦（G 弦）第 2 品 + 食指 — high-G 调弦下最常见按法，与 D 音（3 弦第 2 品）同难度档；② B 音 tips 第三条"和 A 音对比练习"明确鼓励跨音练习，为 P2 评分模块的"音程感知"留自然叙事钩子；③ `built_in_single_notes.dart` 数组末尾追加（不插入中间）保证 T009 既定 carousel 顺序稳定（"do not re-sort in the UI"）；④ `findBuiltInSingleNote('b')` 已知 id 测试从 `'b'`（"未知"）改为 `'h'/'bb'`（仍未知）— 保留 unknown-id 测试意图不丢失；⑤ 所有 length 引用改为 `kBuiltInSingleNotes.length`（dynamic）= `controller_test.dart` `toggleCurrentPracticed counts each note once` 与 `page_test.dart` 进度标签 `'已练习 0 / ${kBuiltInSingleNotes.length}'` 已在 T009 阶段做到 dynamic，6→7 自动跟随；硬编码 length=6 仅出现在 `built_in_single_notes_test.dart` 1 处，已同步更新为 length=7。 |
| **Final Approval** | **Approved**（待 GPT 复审） |
| **Collaboration Value** | **Medium**（T054 为低风险数据追加 + 既有契约保留任务，主会话自审三步反思闭环，未启动独立 Reviewer subagent；与 T053 主会话自审模式一致；Demo A 第三个收口任务（T054 单音系统 7 音）的关键边界（零音频耦合 / 零 chord 耦合 / 零 metronome 耦合 / 既有契约保留 / P2 扩展空间）已通过代码静态分析与 grep 双重验证） |
| **Notes** | ① T054 决策 = `Approved`（主会话自审三步反思 + 测试基线 775 + analyze 0 issues + grep 范围 hygiene）；② Task brief 边界全部遵守：**未**修改任何 metronome / recording / audio 逻辑（`lib/features/metronome/**` `lib/features/recording/**` `lib/features/practice_records/**` `lib/shared/services/real_audio_*` `lib/shared/services/audio_playback_*` `lib/shared/services/audio_file_storage_*` 全部 **未** 触碰）/ Manifest（INTERNET 仍 **未** 声明）/ Drift schema（`schemaVersion = 2` 仍 **未** 升级）/ `pubspec.yaml`（**不**新增依赖 / **不**新增 assets）/ `lesson_constants.dart` / `practice_plan_constants.dart`（Day 3/5/6/7 课程仍冻结；既有 `/single-note` `routePath` 不受影响）/ `router.dart`（`/single-note` 路由 100% KEEP — T054 数据追加不需新增路由）/ `home_quick_actions.dart`（`/single-note` 入口 100% KEEP）/ `SingleNote` / `SingleNoteDifficulty` / `SingleNotePracticeController` / `SingleNotePracticeState` / `SingleNotePracticePage` / `SingleNotePositionDiagram` / `SingleNoteInfoCard` 公共契约 100% KEEP / OP-1 ADR / OP-2 ADR / PRD / SDD / TDD / Implementation Plan 全部 **未** 触碰；③ **决策调整（与 Task brief §5/§7 字面差异）**：经与用户对齐，本任务**不**采用 brief §5/§7 字面要求的 `markCorrect()/markWrong()` state 形状与"播放下一个音 / 正确 / 错误"UI 形状 — 项目 SDD V2 / TDD P1 验收准则（TDD §9.1-T6 + SDD §3.2 P1 ADJUST）明确 T054 只承诺"+ B 音"（ADJUST 而非 rebuild），下游 `practice_plan_constants.dart` / `home_quick_actions.dart` / `practice_records/*` 等 6 个调用方均依赖现有 `SingleNotePracticeController` 公共 API 与 `currentIndex` 形状，重写会破坏契约超出 T054 边界；采用"加 B 音，state/UI 保持原样"路线 = 最大化不动现有代码，最贴合 SDD V2 / TDD P1 验收与下游模块契约；④ **audio 行为**："播放下一个音"按钮按用户决策 = 纯 UI 占位（**不**接 audio），与 brief §6"不接 PCM / 不做 pitch detection"严格一致；⑤ 实施策略：扩展既有 `built_in_single_notes.dart`（追加 1 个 `SingleNote` 实例 B，复用既有 `SingleNote.validate()` 自动校验）+ 注释 + 测试同步（既有 41 个测试 100% 保留通过；T054 新增 1 个 B 音 spot-check 断言 = 42 净增）+ 无 pubspec / 无 assets / 无 Android 配置 / 无路由改动；⑥ **范围清洁度 = 100%**：仅 `lib/features/single_note_practice/**` + `test/features/single_note_practice/**` 范围；`lib/features/metronome/**` `lib/features/recording/**` `lib/features/practice_records/**` `lib/shared/services/real_audio_*` `lib/shared/services/audio_playback_*` `lib/shared/services/audio_file_storage_*` `lib/features/chord_library/**` `lib/features/chord_practice/**` `lib/features/tuner/**` `lib/features/home/**` `lib/features/lesson_c_am_down_4x4/**` 全部 **未** 触碰；⑦ 测试基线 774 → 775 净增 1（T009 既有的 20 项 built_in_single_notes + 9 项 controller + 6 项 page + 3 项 page layout + 3 项 position diagram = 41 项全部保留通过；T054 新增 1 项 B 音 spot-check = 42 项）；⑧ Reviewer 协作机制说明：T054 采用主会话自审 + 三步反思模式，与 T053 一致；Demo A 真机整合测试阶段可启动 3 名 Reviewer（Flutter Architecture / Product Alignment / Audio / Pitch Safety）做更深入评审；⑨ 不启动 OP-1 / OP-2 真机 Spike（T054 不依赖 OP-1 / OP-2 决策门，单纯是单音数据补齐到 7）；⑩ 不启动 P2 音高识别 / pitch detection（T054 任务 brief §3 明确"P2 留扩展空间但不实现"）；⑪ 不 push / 不 Tag / 不 amend / rebase / reset --hard；⑫ T054 起始 HEAD = `c46d0a8`（T053 提交）、branch = `product-v2`、working tree clean；T054 提交后预期新增 `feat: add 7th single note B for demo A closure` 一个 commit；⑬ **下一阶段** = GPT 首席架构师决定是否启动 Demo A 真机整合测试（T052 节拍器可听 + T053 7 和弦切换 + T054 7 单音切换三闭环在 HUAWEI CDY-AN90 / Android 10 真机上一并验证）；⑭ Demo A 录屏需用户在真机上验证：① 节拍器 80 BPM 4/4 可听 + 重拍/弱拍区分清晰；② 7 和弦切换 UI 流畅 + 指法图渲染正确 + 选中态高亮；③ 7 单音（**含 B 音**）切换 UI 流畅 + 位置图渲染正确 + "标记已练习 / 取消已练习" 切换闭环 + 进度计数 0/7 → 7/7；⑮ 若 GPT 复审要求把 B 音改为 2 弦第 3 品（中指）或 1 弦第 2 品（食指），非 Blocker — 任务 brief §5.1 仅指定"C D E F G A B 七个"，**未**指定 B 音的具体按法，本任务选择"4 弦第 2 品 / 食指"基于 high-G 调弦最常见 beginner 按法 + 与 A 音（1 弦空弦）形成"同弦跨品"对照练习友好性。 |