...
| Final Approval | **PENDING 真机用户确认**（自动化 744 / 744 + analyze 0 issue；本次提交**不**含 Approved 标注，必须在用户原设备执行 `adb install -r` 覆盖安装后点击"开始课程"确认课程页真实显示，由用户手动标记 PASS） |
...
| Notes | 详见 `docs/dev/TASK_LEDGER.md` T045A 节段 + `docs/qa/T045_LESSON_ANDROID_ACCEPTANCE.md` T045A 修复交付记录；本次提交**不** push / **不**创建 Tag / **未** amend / rebase / reset --hard；**未**修改除 `lib/app/router.dart` 外的任何生产代码 / **未**修改任何路由或课程功能 / **未**修改既有 740 项测试 / **未**撤销权限 / **未**卸载或清除数据；真机由用户在原设备执行 `adb install -r build/app/outputs/flutter-apk/app-debug.apk`（**禁止** `flutter install`）覆盖安装后手动标记 PASS |
