...
| Task ID | 任务名称 | 状态 | Commit | 测试总数 | GPT 复审 | 遗留说明 |
| --- | --- | --- | --- | --- | --- | --- |
| T014_BUILD_MVP_PRACTICE_RECORD_FLOW_TEST | MVP 练习记录完整纵向流程集成测试 | 通过 | `b17e780` | 407 | 待复审 | 新增 `mvp_practice_record_flow_test.dart`,将首页安装日期 / 今日任务完成 → 模拟录音保存 → 记录列表展示 → 详情展示 → 删除 5 个环节串成一条 E2E 路径,共用同一份真实 `AppDatabase` / Repository / Provider 容器;新增测试 10 项,最终测试数 397 + 10 − 0 = 407;无生产代码改动 |
| T014_ANDROID_BUILD_PRECHECK | Android 构建工具链与 Manifest 预检 | 通过 | `4e9dcec` / `7407660` / `87d854d` | 407 | 待复审 | 三提交共同完成 Android 构建预检:声明 Flutter v2 embedding(`7407660`)、校准 AGP / Kotlin / Gradle 工具链(`4e9dcec`)、按离线 MVP 重新整理 `AndroidManifest.xml`(`87d854d`);无 INTERNET 与 RECORD_AUDIO 权限,只保留 launcher Activity;与本任务之前已存在的产品代码无接口变更 |
| T015_REMOVE_UNUSED_AUDIO_AND_PERMISSION_DEPS | 依赖与 Manifest 清理(录音 / 音频 / sqlite3 / 权限) | 通过 | `d93ae39` / `5e26282` / `146686d` | 407 | 待复审 | 三提交共同完成离线 MVP 范围收口:移除录音 / 音频 / 权限相关 pubspec 依赖与对应 `pubspec.lock` 项(`d93ae39` + `5e26282`),同步清理 `AndroidManifest.xml` 中已无意义的条目,移除 EOL sqlite3 flutter libs(`146686d`);最终 `pubspec.yaml` 不再依赖 record / flutter_sound / permission_handler / sqlite3_flutter_libs 等与当前 MVP 无关的包;本任务无生产行为改动,仅删除依赖与 Manifest 噪音 |
| T015_ANDROID_DEVICE_ACCEPTANCE | Android 真机验收与体验文案 / 布局打磨 | 通过 | `ebfe12d` | 407 | 待复审 | 用户已在真机 `CDY-AN90` 上完成覆盖安装、启动、杀进程重启、强行停止重启等验收项;`ebfe12d` 同步修复首页 / 关于 / 内容声明 / 隐私声明 / 设置入口 / 单音练习页文案与布局,新增对应 widget / copy 测试覆盖;**验收项本身由用户手工完成,代码侧只承担文案与布局打磨**;真机验收的逐项结论汇总在 `docs/dev/MVP_ACCEPTANCE.md` |
...
> 说明:T014-T016 段明确区分"自动测试 / 构建验证"与"用户手工真机验收"两类事实,任何用户手工验收的逐项结论请见 `docs/dev/MVP_ACCEPTANCE.md`,本台账不重复展开。Release APK / AAB 构建、正式签名配置、商店发布流程、iOS 验收、真实音频能力均不在 T016 范围内,不得视作完成。