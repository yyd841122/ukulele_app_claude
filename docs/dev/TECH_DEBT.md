...
| TD-010 | 真实音频阶段必须新增或评估的音频相关依赖(`record` 7.1.0 / `just_audio` 0.10.5 / `permission_handler` 12.0.3 / `path_provider` 2.1.6)已由 T026 完成依赖研究 / Spike 调研;**T027 已完成 `permission_handler ^12.0.3` 引入**(TDD 完成:14 项单元测试覆盖 6 状态映射 + 3 契约不变式 + openSettings 真假分支;三处 AndroidManifest 显式声明 `RECORD_AUDIO`);`record ^7.1.0` 与 `just_audio ^0.10.5` 仍**未**写入 `pubspec.yaml`,必须由 T029 / T030 两个独立任务按"一个任务一个 commit"节奏执行,**不得**绕过 Spike 直接恢复旧依赖或一次性引入多个依赖 | 高 — T026 已提供研究结论,T027 已完成 `permission_handler` 引入;T029 / T030 仍待 GPT 首席架构师独立 Prompt | 高 | T027 已完成; T029 (`record`) / T030 (`just_audio`) 待 GPT 首席架构师独立 Prompt | 待处理 (T026 Spike 完成,T027 已引入 `permission_handler ^12.0.3` + 三处 Manifest 声明 `RECORD_AUDIO` + 权限服务 + 14 项单元测试;`record ^7.1.0` 与 `just_audio ^0.10.5` 仍**未**写入 `pubspec.yaml`;待 GPT 首席架构师下发 T029 / T030 独立 Prompt 后才能启动) |
