# T045 Lesson Android Acceptance — CLOSED (T045A PASS)

> 第一节课程（`/lessons/c_am_down_4x4`）真机验收在第 1 步即失败。
> 修复交由独立任务 `T045A_FIX_LESSON_CHILD_ROUTE_REDIRECT`，在 commit `d707313` 落地。
> **2026-06-24 真机复测 PASS**（用户在原设备覆盖安装后手动确认全部 4 项行为）。
...
| T045 起始 HEAD | `6b57669d8a2f96afb60c0ce28e2ffd754288b112` |
| T045 结束 HEAD | `6b57669d8a2f96afb60c0ce28e2ffd754288b112`（**未**前进；BLOCKED） |
| T045A 起始 HEAD | `6b57669d8a2f96afb60c0ce28e2ffd754288b112` |
| T045A 修复 HEAD | `d707313738609fa7052f1b93b7e8a8f216d29e59`（`fix: allow lesson child route navigation`） |
| 最终 HEAD（push 后） | `d707313`（fast-forward，origin/master 同步） |
| 测试基线 | 740（BLOCKED）→ 744（T045A 修复后：+4 e2e） |
| 设备 | HUAWEI CDY-AN90 / Android 10（**未**输出完整序列号） |
| 自动化测试 | `flutter analyze` 0 issue；`flutter test` 744 / 744 通过 |
| 真机验收（2026-06-24） | **PASS**（用户手动确认） |
| 状态 | **CLOSED**（T045 收口；T045A 端到端 PASS） |
...
## 验收范围（T045 原始 FAIL → T045A 真机 PASS）

| 步骤 | T045 状态 | T045A 真机状态（2026-06-24） |
| --- | --- | --- |
| 1. C 和弦页课程入口可见 | ✅ 视觉确认（用户） | ✅ 保留 |
| 2. 课程页面内容与节奏图正常 | ❌ **未到达**（步骤 1 push 后被 redirect 回首页） | ✅ **PASS**（页面真实显示，标题/步骤/StrumPatternDiagram/C-Am 指法参考齐全） |
| 3. 页面滚动及文字无截断 | ❌ 未到达 | ✅ **PASS**（用户确认） |
| 4. 节拍器入口与返回正常，未自动启动或改 BPM | ❌ 未到达 | ✅ **PASS**（用户确认入口打开且未自动启动） |
| 5. 录音入口与返回正常，未自动录音 | ❌ 未到达 | ✅ **PASS**（用户确认入口打开且未自动录音） |
| 6. 原有练习记录和音频仍存在 | ⚠️ **N/A**（前期 `flutter install` 卸载了旧版，数据已清空；T045A `adb install -r` 覆盖安装**不**改变这一事实） | ⚠️ **N/A**（数据已在前置阶段不可恢复地丢失；T045A 范围**不**得声称 PASS——验收口径保持 N/A） |
| 7. 无崩溃或明显 UI 异常 | ✅ 视觉确认无崩溃，仅路由错误 | ✅ **PASS**（修复后路由正常） |
...
### 真机验收（2026-06-24，用户手动 PASS）

用户在原设备（HUAWEI CDY-AN90 / Android 10）执行 `adb install -r build/app/outputs/flutter-apk/app-debug.apk`（**未** `flutter install` / **未**卸载 / **未**清数据 / **未**撤权限）覆盖安装后，按 T044 验收矩阵逐项确认：

1. C 和弦页点击"开始课程" → **课程详情页真实显示**，不再跳回主页。
2. 课程页 AppBar 返回箭头 → **正常 pop 回 C 和弦页**。
3. 课程页"调整到 60 BPM 后开始" → **节拍器页打开**，未自动启动、未改 BPM。
4. 课程页"录音复盘" → **录音页打开**，未自动开始录音。

用户口头确认全部 4 项 PASS，T045A 修复在真机层面闭环。

### 数据保留状态（最终口径）

T045 早期真机阶段使用了 `flutter install`，按 Android 平台行为已卸载旧版并清空应用数据；旧版（含未备份的练习记录、录音音频、节拍器设置、安装日期等）**已不可恢复地丢失**。T045A 仅修改路由配置 + 新增测试，且真机步骤仅使用 `adb install -r` 覆盖安装（**不**卸载 / **不**清数据），**不**改变这一事实。验收表第 6 项保持 **N/A**，**不得**改写为通过；任何后续任务（lesson-2 / T046+）若声称"数据保留 PASS"均与本文档冲突。