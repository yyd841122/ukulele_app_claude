...
//   / SelfRatingSelector / NoteField / ResetButton) so this file
//   stays short and each piece is independently testable via
//   `find.byKey(...)`.
...
/// progress (the user must stop first) or when nothing has been
...
class ResetButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bool canReset =
        !state.isRecording && (state.hasRecording || state.isPlaying);
...
class SelfRatingSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SegmentedButton<SelfRating>(
      ...
      onSelectionChanged: (Set<SelfRating> selection) {
        // SegmentedButton with multi=false (default) returns a set
        // of size 0 or 1. Forward the single value (or null) to
        // the controller.
